...
public class DiskBoundaryManager
{
    ...
    private static DiskBoundaries getDiskBoundaryValue(ColumnFamilyStore cfs)
    {
        ...
        List<Range<Token>> sortedFullLocalRanges = localRanges.stream()
                                                              .filter(Replica::isFull)
                                                              .map(Replica::range)
                                                              .sorted(Range.leftComparator)
                                                              .collect(Collectors.toList());
        List<Range<Token>> sortedTransientLocalRanges = localRanges.stream()
                                                                   .filter(Replica::isFull)
                                                                   .map(Replica::range)
                                                                   .sorted(Range.leftComparator)
                                                                   .collect(Collectors.toList());

        List<PartitionPosition> positions = getDiskBoundaries(sortedFullLocalRanges, sortedTransientLocalRanges, cfs.getPartitioner(), dirs);
     ...
     */
    private static List<PartitionPosition> getDiskBoundaries(List<Range<Token>> sortedFullRanges, List<Range<Token>> sortedTransientRanges, IPartitioner partitioner, Directories.DataDirectory[] dataDirectories)
    {
        assert partitioner.splitter().isPresent();
        Splitter splitter = partitioner.splitter().get();
        boolean dontSplitRanges = DatabaseDescriptor.getNumTokens() > 1;

        List<Splitter.WeightedRange> weightedRanges = new ArrayList<>(sortedFullRanges.size() + (sortedTransientRanges != null ? sortedTransientRanges.size() : 0));
        for (Range<Token> r : sortedFullRanges)
            weightedRanges.add(new Splitter.WeightedRange(1.0, r));

        if (sortedTransientRanges != null)
        {
            for (Range<Token> r : sortedTransientRanges)
                weightedRanges.add(new Splitter.WeightedRange(0.1, r));
        }

        weightedRanges.sort(Comparator.comparing(r -> r.left(splitter)));
