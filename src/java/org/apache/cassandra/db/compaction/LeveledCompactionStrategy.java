...
public class LeveledCompactionStrategy extends AbstractCompactionStrategy
{
            ...
            for (Integer level : byLevel.keySet())
            {
                ...
                if (level <= 0)
                {
                    ...
                    for (SSTableReader sstable : byLevel.get(level))
                        scanners.add(sstable.getScanner(range, CompactionManager.instance.getRateLimiter(), true));
                }
                else
                {
    ...
    private static class LeveledScanner extends AbstractIterator<OnDiskAtomIterator> implements ICompactionScanner
    {
        ...
        public LeveledScanner(Collection<SSTableReader> sstables, Range<Token> range)
        {
            ...
            currentScanner = sstableIterator.next().getScanner(range, CompactionManager.instance.getRateLimiter(), true);
        }
