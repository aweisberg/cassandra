...
{
    private void reply(int id, InetAddressAndPorts replyTo)
    {
    ...
    public void doVerb(MessageIn<Mutation> message, int id)  throws IOException
    {
        ...
        InetAddressAndPorts from = (InetAddressAndPorts)message.parameters.get(ParameterType.FORWARD_FROM);
        InetAddressAndPorts replyTo;
        if (from == null)
        {
    ...
    private static void forwardToLocalNodes(Mutation mutation, MessagingService.Verb verb, ForwardToContainer forwardTo, InetAddressAndPorts from) throws IOException
    {
        ...
        Iterator<InetAddressAndPorts> iterator = forwardTo.targets.iterator();
        // Send a message to each of the addresses on our Forward List
        for (int i = 0; i < forwardTo.targets.size(); i++)
        {
            InetAddressAndPorts address = iterator.next();
