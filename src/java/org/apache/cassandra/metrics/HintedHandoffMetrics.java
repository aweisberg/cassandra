...
public class HintedHandoffMetrics
{
    ...
    private final LoadingCache<InetAddressAndPorts, DifferencingCounter> notStored = Caffeine.newBuilder()
         .executor(MoreExecutors.directExecutor())
         .build(DifferencingCounter::new);

    /** Total number of hints that have been created, This is not a cache. */
    private final LoadingCache<InetAddressAndPorts, Counter> createdHintCounts = Caffeine.newBuilder()
         .executor(MoreExecutors.directExecutor())
         .build(address -> Metrics.counter(factory.createMetricName("Hints_created-" + address.toString().replace(':', '.'))));

    public void incrCreatedHints(InetAddressAndPorts address)
    {
        createdHintCounts.get(address).inc();
    }

    public void incrPastWindow(InetAddressAndPorts address)
    {
    ...
    public void log()
    {
        for (Entry<InetAddressAndPorts, DifferencingCounter> entry : notStored.asMap().entrySet())
        {
    ...
    public static class DifferencingCounter
    {
        ...
        public DifferencingCounter(InetAddressAndPorts address)
        {
