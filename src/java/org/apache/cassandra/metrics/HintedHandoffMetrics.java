...
public class HintedHandoffMetrics
{
    ...
    private final LoadingCache<Endpoint, DifferencingCounter> notStored = Caffeine.newBuilder()
                                                                                  .executor(MoreExecutors.directExecutor())
                                                                                  .build(DifferencingCounter::new);

    /** Total number of hints that have been created, This is not a cache. */
    private final LoadingCache<Endpoint, Counter> createdHintCounts = Caffeine.newBuilder()
                                                                              .executor(MoreExecutors.directExecutor())
                                                                              .build(address -> Metrics.counter(factory.createMetricName("Hints_created-" + address.toString().replace(':', '.'))));

    public void incrCreatedHints(Endpoint address)
    {
        createdHintCounts.get(address).inc();
    }

    public void incrPastWindow(Endpoint address)
    {
    ...
    public void log()
    {
        for (Entry<Endpoint, DifferencingCounter> entry : notStored.asMap().entrySet())
        {
    ...
    public static class DifferencingCounter
    {
        ...
        public DifferencingCounter(Endpoint address)
        {
