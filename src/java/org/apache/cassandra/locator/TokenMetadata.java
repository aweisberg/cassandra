...
public class TokenMetadata
{
     ...
     */
    public void addIfMissing(Endpoint endpoint)
    {
     ...
     * cannot be changed after the fact.
     */
    public void updateHostId(UUID hostId, Endpoint endpoint)
    {
        assert hostId != null;
        assert endpoint != null;

        final Endpoint newEndpoint = Endpoint.getByAddressOverrideDefaults(endpoint.address, endpoint.port, hostId);
        ...
        try
        {
            ...
            {
                Set<Endpoint> storedEndpointsWithAddress = allEndpoints.stream().filter(e -> e.equalAddresses(endpoint)).collect(Collectors.toSet());

                // Check if another node with the same IP/Port combo is already running
                for (Endpoint ep : storedEndpointsWithAddress)
                ...
                Optional<Endpoint> uninitializedEndpoint = storedEndpointsWithAddress.stream().filter(e -> e.hostId.equals(Endpoint.initialHostId)).findFirst();
                                                           ...
                                                           Set<Pair<Token, Endpoint>> movingEndpoints)
    {
        ...
        Multimap<Endpoint, Range<Token>> addressRanges = strategy.getAddressRanges(metadata);
