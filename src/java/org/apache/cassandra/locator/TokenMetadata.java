...
public class TokenMetadata
{
     ...
     */
    private final BiMultiValMap<Token, InetAddressAndPorts> tokenToEndpointMap;

    /** Maintains endpoint to host ID map of every node in the cluster */
    private final BiMap<InetAddressAndPorts, UUID> endpointToHostIdMap;

    // Prior to CASSANDRA-603, we just had <tt>Map<Range, InetAddressAndPorts> pendingRanges<tt>,
    ...
    //
    // First, we changed pendingRanges to a <tt>Multimap<Range, InetAddressAndPorts></tt> (now
    // <tt>Map<String, Multimap<Range, InetAddressAndPorts>></tt>, because replication strategy
    ...
    private final BiMultiValMap<Token, InetAddressAndPorts> bootstrapTokens = new BiMultiValMap<>();

    private final BiMap<InetAddressAndPorts, InetAddressAndPorts> replacementToOriginal = HashBiMap.create();

    // (don't need to record Token here since it's still part of tokenToEndpointMap until it's done leaving)
    private final Set<InetAddressAndPorts> leavingEndpoints = new HashSet<>();
    ...
    private final Set<Pair<Token, InetAddressAndPorts>> movingEndpoints = new HashSet<>();
    ...
    {
        this(SortedBiMultiValMap.<Token, InetAddressAndPorts>create(),
             HashBiMap.create(),
             new Topology(),
             DatabaseDescriptor.getPartitioner());
    }

    private TokenMetadata(BiMultiValMap<Token, InetAddressAndPorts> tokenToEndpointMap, BiMap<InetAddressAndPorts, UUID> endpointsMap, Topology topology, IPartitioner partitioner)
    {
    ...
    public int pendingRangeChanges(InetAddressAndPorts source)
    {
        int n = 0;
     ...
     */
    public void updateNormalToken(Token token, InetAddressAndPorts endpoint)
    {
        updateNormalTokens(Collections.singleton(token), endpoint);
    }

    public void updateNormalTokens(Collection<Token> tokens, InetAddressAndPorts endpoint)
    {
        Multimap<InetAddressAndPorts, Token> endpointTokens = HashMultimap.create();
     ...
     */
    public void updateNormalTokens(Multimap<InetAddressAndPorts, Token> endpointTokens)
    {
        ...
        try
        {
            ...
            for (InetAddressAndPorts endpoint : endpointTokens.keySet())
            {
                ...
                for (Token token : tokens)
                {
                    InetAddressAndPorts prev = tokenToEndpointMap.put(token, endpoint);
     ...
     */
    public void updateHostId(UUID hostId, InetAddressAndPorts endpoint)
    {
        assert hostId != null;
        ...
        try
        {
            InetAddressAndPorts storedEp = endpointToHostIdMap.inverse().get(hostId);
            if (storedEp != null)
            {
    ...
    public Map<InetAddressAndPorts, UUID> getEndpointToHostIdMapForReading()
    {
        lock.readLock().lock();
        try
        {
            Map<InetAddressAndPorts, UUID> readMap = new HashMap<>();
    ...
    @Deprecated
    public void addBootstrapToken(Token token, InetAddressAndPorts endpoint)
    {
        addBootstrapTokens(Collections.singleton(token), endpoint);
    }

    public void addBootstrapTokens(Collection<Token> tokens, InetAddressAndPorts endpoint)
    {
        addBootstrapTokens(tokens, endpoint, null);
    }

    private void addBootstrapTokens(Collection<Token> tokens, InetAddressAndPorts endpoint, InetAddressAndPorts original)
    {
        assert tokens != null && !tokens.isEmpty();
        assert endpoint != null;
        ...
        try
        {
            ...
            InetAddressAndPorts oldEndpoint;
    ...
    public void addReplaceTokens(Collection<Token> replacingTokens, InetAddressAndPorts newNode, InetAddressAndPorts oldNode)
    {
    ...
    public Optional<InetAddressAndPorts> getReplacementNode(InetAddressAndPorts endpoint)
    {
        return Optional.ofNullable(replacementToOriginal.inverse().get(endpoint));
    }

    public Optional<InetAddressAndPorts> getReplacingNode(InetAddressAndPorts endpoint)
    {
    ...
    public void addLeavingEndpoint(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
     ...
     */
    public void addMovingEndpoint(Token token, InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
     ...
     * @param endpoint address of the moving node
     */
    public void removeFromMoving(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
        ...
        try
        {
            for (Pair<Token, InetAddressAndPorts> pair : movingEndpoints)
            {
    ...
    public Collection<Token> getTokens(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
    ...
    @Deprecated
    public Token getToken(InetAddressAndPorts endpoint)
    {
        return getTokens(endpoint).iterator().next();
    }

    public boolean isMember(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
    ...
    public boolean isLeaving(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
    ...
    public boolean isMoving(InetAddressAndPorts endpoint)
    {
        assert endpoint != null;
    ...
    private static TokenMetadata removeEndpoints(TokenMetadata allLeftMetadata, Set<InetAddressAndPorts> leavingEndpoints)
    {
        for (InetAddressAndPorts endpoint : leavingEndpoints)
    ...
    public InetAddressAndPorts getEndpoint(Token token)
    {
        lock.readLock().lock();
        try
        {
    ...
    public Multimap<Range<Token>, InetAddressAndPorts> getPendingRangesMM(String keyspaceName)
    {
        ...
        {
            for (Map.Entry<Range<Token>, List<InetAddressAndPorts>> entry : pendingRangeMaps)
            {
                Range<Token> range = entry.getKey();
                for (InetAddressAndPorts address : entry.getValue())
                {
    ...
    public List<Range<Token>> getPendingRanges(String keyspaceName, InetAddressAndPorts endpoint)
    {
        List<Range<Token>> ranges = new ArrayList<>();
        for (Map.Entry<Range<Token>, InetAddressAndPorts> entry : getPendingRangesMM(keyspaceName).entries())
        {
    ...
    public void calculatePendingRanges(AbstractReplicationStrategy strategy, String keyspaceName)
    {
        ...
        synchronized (pendingRanges)
        {
            ...
            else
            {
                ...
                BiMultiValMap<Token, InetAddressAndPorts> bootstrapTokens = new BiMultiValMap<>();
                Set<InetAddressAndPorts> leavingEndpoints = new HashSet<>();
                Set<Pair<Token, InetAddressAndPorts>> movingEndpoints = new HashSet<>();
    ...
    private static PendingRangeMaps calculatePendingRanges(AbstractReplicationStrategy strategy,
                                                           TokenMetadata metadata,
                                                           BiMultiValMap<Token, InetAddressAndPorts> bootstrapTokens,
                                                           Set<InetAddressAndPorts> leavingEndpoints,
                                                           Set<Pair<Token, InetAddressAndPorts>> movingEndpoints)
    {
        PendingRangeMaps newPendingRanges = new PendingRangeMaps();

        Multimap<InetAddressAndPorts, Range<Token>> addressRanges = strategy.getAddressRanges(metadata);
        ...
        {
            Set<InetAddressAndPorts> currentEndpoints = ImmutableSet.copyOf(strategy.calculateNaturalEndpoints(range.right, metadata));
            Set<InetAddressAndPorts> newEndpoints = ImmutableSet.copyOf(strategy.calculateNaturalEndpoints(range.right, allLeftMetadata));
        ...
        Multimap<InetAddressAndPorts, Token> bootstrapAddresses = bootstrapTokens.inverse();
        for (InetAddressAndPorts endpoint : bootstrapAddresses.keySet())
        {
        ...
        for (Pair<Token, InetAddressAndPorts> moving : movingEndpoints)
        {
            ...
            InetAddressAndPorts endpoint = moving.right; // address of the moving node
            ...
            for(Range<Token> range : moveAffectedRanges)
            {
                Set<InetAddressAndPorts> currentEndpoints = ImmutableSet.copyOf(strategy.calculateNaturalEndpoints(range.right, metadata));
                Set<InetAddressAndPorts> newEndpoints = ImmutableSet.copyOf(strategy.calculateNaturalEndpoints(range.right, allLeftMetadata));
                Set<InetAddressAndPorts> difference = Sets.difference(newEndpoints, currentEndpoints);
                for(final InetAddressAndPorts address : difference)
                {
    ...
    public BiMultiValMap<Token, InetAddressAndPorts> getBootstrapTokens()
    {
        lock.readLock().lock();
        try
        {
    ...
    public Set<InetAddressAndPorts> getAllEndpoints()
    {
        lock.readLock().lock();
        try
        {
    ...
    public Set<InetAddressAndPorts> getLeavingEndpoints()
    {
        lock.readLock().lock();
        try
        {
     ...
     */
    public Set<Pair<Token, InetAddressAndPorts>> getMovingEndpoints()
    {
        lock.readLock().lock();
        try
        {
    ...
    public String toString()
    {
        ...
        try
        {
            Multimap<InetAddressAndPorts, Token> endpointToTokenMap = tokenToEndpointMap.inverse();
            Set<InetAddressAndPorts> eps = endpointToTokenMap.keySet();
            ...
            if (!bootstrapTokens.isEmpty())
            {
                ...
                sb.append(System.getProperty("line.separator"));
                for (Map.Entry<Token, InetAddressAndPorts> entry : bootstrapTokens.entrySet())
                {
            ...
            if (!leavingEndpoints.isEmpty())
            {
                ...
                sb.append(System.getProperty("line.separator"));
                for (InetAddressAndPorts ep : leavingEndpoints)
                {
                    sb.append(ep);
                    sb.append(System.getProperty("line.separator"));
                }
    ...
    public Collection<InetAddressAndPorts> pendingEndpointsFor(Token token, String keyspaceName)
    {
        PendingRangeMaps pendingRangeMaps = this.pendingRanges.get(keyspaceName);
        if (pendingRangeMaps == null)
     ...
     */
    public Collection<InetAddressAndPorts> getWriteEndpoints(Token token, String keyspaceName, Collection<InetAddressAndPorts> naturalEndpoints)
    {
        return ImmutableList.copyOf(Iterables.concat(naturalEndpoints, pendingEndpointsFor(token, keyspaceName)));
    }

    /** @return an endpoint to token multimap representation of tokenToEndpointMap (a copy) */
    public Multimap<InetAddressAndPorts, Token> getEndpointToTokenMapForReading()
    {
        lock.readLock().lock();
        try
        {
            Multimap<InetAddressAndPorts, Token> cloned = HashMultimap.create();
            for (Map.Entry<Token, InetAddressAndPorts> entry : tokenToEndpointMap.entrySet())
     ...
     *         in the cluster.
     */
    public Map<Token, InetAddressAndPorts> getNormalAndBootstrappingTokenToEndpointMap()
    {
        lock.readLock().lock();
        try
        {
            Map<Token, InetAddressAndPorts> map = new HashMap<>(tokenToEndpointMap.size() + bootstrapTokens.size());
    ...
    public static class Topology
    {
        ...
        private final Multimap<String, InetAddressAndPorts> dcEndpoints;
        /** map of DC to multi-map of rack to endpoints in that rack */
        private final Map<String, Multimap<String, InetAddressAndPorts>> dcRacks;
        /** reverse-lookup map for endpoint to current known dc/rack assignment */
        private final Map<InetAddressAndPorts, Pair<String, String>> currentLocations;
         ...
         */
        void addEndpoint(InetAddressAndPorts ep)
        {
        ...
        private void doAddEndpoint(InetAddressAndPorts ep, String dc, String rack)
        {
         ...
         */
        void removeEndpoint(InetAddressAndPorts ep)
        {
        ...
        private void doRemoveEndpoint(InetAddressAndPorts ep, Pair<String, String> current)
        {
            dcRacks.get(current.left).remove(current.right, ep);
            dcEndpoints.remove(current.left, ep);
        }

        void updateEndpoint(InetAddressAndPorts ep)
        {
            IEndpointSnitch snitch = DatabaseDescriptor.getEndpointSnitch();
        ...
        void updateEndpoints()
        {
            ...
            for (InetAddressAndPorts ep : currentLocations.keySet())
                updateEndpoint(ep, snitch);
        }

        private void updateEndpoint(InetAddressAndPorts ep, IEndpointSnitch snitch)
        {
            Pair<String, String> current = currentLocations.get(ep);
            String dc = snitch.getDatacenter(ep);
            String rack = snitch.getRack(ep);
         ...
         */
        public Multimap<String, InetAddressAndPorts> getDatacenterEndpoints()
        {
            return dcEndpoints;
        }
         ...
         */
        public Map<String, Multimap<String, InetAddressAndPorts>> getDatacenterRacks()
        {
            return dcRacks;
        }
         ...
         */
        public Pair<String, String> getLocation(InetAddressAndPorts addr)
        {
