...
public class GossipDigestAck2
{
    ...
    GossipDigestAck2(Map<InetAddressAndPorts, EndpointState> epStateMap)
    {
...
class GossipDigestAck2Serializer implements IVersionedSerializer<GossipDigestAck2>
{
    public void serialize(GossipDigestAck2 ack2, DataOutputPlus out, int version) throws IOException
    {
        ...
        for (Map.Entry<InetAddressAndPorts, EndpointState> entry : ack2.epStateMap.entrySet())
        {
    ...
    {
        int size = in.readInt();
        Map<InetAddressAndPorts, EndpointState> epStateMap = new HashMap<>(size);
