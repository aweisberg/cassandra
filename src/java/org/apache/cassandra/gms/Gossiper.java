...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean
{
    ...
    /* live member set */
    private final Set<InetAddressAndPorts> liveEndpoints = new ConcurrentSkipListSet<>();

    /* unreachable member set */
    private final Map<InetAddressAndPorts, Long> unreachableEndpoints = new ConcurrentHashMap<>();

    /* initial seeds for joining the cluster */
    private final Set<InetAddressAndPorts> seeds = new ConcurrentSkipListSet<>();

    /* map where key is the endpoint and value is the state associated with the endpoint */
    final ConcurrentMap<InetAddressAndPorts, EndpointState> endpointStateMap = new ConcurrentHashMap<>();

    /* map where key is endpoint and value is timestamp when this endpoint was removed from
     * gossip. We will ignore any gossip regarding these endpoints for QUARANTINE_DELAY time
     * after removal to prevent nodes from falsely reincarnating during the time when removal
     * gossip gets propagated to all nodes */
    private final Map<InetAddressAndPorts, Long> justRemovedEndpoints = new ConcurrentHashMap<>();

    private final Map<InetAddressAndPorts, Long> expireTimeEndpointMap = new ConcurrentHashMap<>();

    private volatile boolean inShadowRound = false;
    private final Set<InetAddressAndPorts> seedsInShadowRound = new ConcurrentSkipListSet<>();
    ...
    private class GossipTask implements Runnable
    {
        public void run()
        {
            try
            {
                ...
                endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts()).getHeartBeatState().updateHeartBeat();
                if (logger.isTraceEnabled())
                    logger.trace("My heartbeat is now {}", endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts()).getHeartBeatState().getHeartBeatVersion());
    ...
    {
        for (Map.Entry<InetAddressAndPorts, EndpointState> entry : endpointStateMap.entrySet())
        {
            ...
            try
            {
                ...
                VersionedValue internalIpAndPorts = entry.getValue().getApplicationState(ApplicationState.INTERNAL_ADDRESS_AND_PORTS);
                InetAddressAndPorts endpoint = null;
                if (internalIpAndPorts != null)
                {
                    endpoint = InetAddressAndPorts.getByName(internalIpAndPorts.value);
                }
                else if (internalIp != null)
                {
                    endpoint = InetAddressAndPorts.getByName(internalIp.value);
                }
     ...
     */
    public Set<InetAddressAndPorts> getLiveMembers()
    {
        Set<InetAddressAndPorts> liveMembers = new HashSet<>(liveEndpoints);
        if (!liveMembers.contains(FBUtilities.getBroadcastAddressAndPorts()))
            liveMembers.add(FBUtilities.getBroadcastAddressAndPorts());
        return liveMembers;
    }
     ...
     */
    public Set<InetAddressAndPorts> getLiveTokenOwners()
    {
     ...
     */
    public Set<InetAddressAndPorts> getUnreachableMembers()
    {
     ...
     */
    public Set<InetAddressAndPorts> getUnreachableTokenOwners()
    {
        Set<InetAddressAndPorts> tokenOwners = new HashSet<>();
        for (InetAddressAndPorts endpoint : unreachableEndpoints.keySet())
        {
    ...
    public long getEndpointDowntime(InetAddressAndPorts ep)
    {
    ...
    private boolean isShutdown(InetAddressAndPorts endpoint)
    {
        ...
        VersionedValue versionedValue = epState.getApplicationState(ApplicationState.STATUS_WITH_PORTS);
     ...
     */
    protected void markAsShutdown(InetAddressAndPorts endpoint)
    {
        EndpointState epState = endpointStateMap.get(endpoint);
        if (epState == null)
            return;
        epState.addApplicationState(ApplicationState.STATUS_WITH_PORTS, StorageService.instance.valueFactory.shutdown(true));
     ...
     */
    private void evictFromMembership(InetAddressAndPorts endpoint)
    {
     ...
     */
    public void removeEndpoint(InetAddressAndPorts endpoint)
    {
     ...
     * @param endpoint
     */
    private void quarantineEndpoint(InetAddressAndPorts endpoint)
    {
     ...
     */
    private void quarantineEndpoint(InetAddressAndPorts endpoint, long quarantineExpiration)
    {
     ...
     * @param endpoint
     */
    public void replacementQuarantine(InetAddressAndPorts endpoint)
    {
     ...
     */
    public void replacedEndpoint(InetAddressAndPorts endpoint)
    {
    ...
    private void makeRandomGossipDigest(List<GossipDigest> gDigests)
    {
        ...
        List<InetAddressAndPorts> endpoints = new ArrayList<>(endpointStateMap.keySet());
     ...
     */
    public void advertiseRemoving(InetAddressAndPorts endpoint, UUID hostId, UUID localHostId)
    {
        EndpointState epState = endpointStateMap.get(endpoint);
        ...
        states.put(ApplicationState.STATUS_WITH_PORTS, StorageService.instance.valueFactory.removingNonlocal(hostId));
     ...
     * @param hostId
     */
    public void advertiseTokenRemoved(InetAddressAndPorts endpoint, UUID hostId)
    {
        EndpointState epState = endpointStateMap.get(endpoint);
        epState.updateTimestamp(); // make sure we don't evict it too soon
        epState.getHeartBeatState().forceNewerGenerationUnsafe();
        long expireTime = computeExpireTime();
        epState.addApplicationState(ApplicationState.STATUS_WITH_PORTS, StorageService.instance.valueFactory.removedNonlocal(hostId, expireTime));
    ...
    {
        InetAddressAndPorts endpoint = InetAddressAndPorts.getByName(address);
        EndpointState epState = endpointStateMap.get(endpoint);
        ...
        epState.addApplicationState(ApplicationState.STATUS_WITH_PORTS, StorageService.instance.valueFactory.left(tokens, computeExpireTime()));
    ...
    public boolean isKnownEndpoint(InetAddressAndPorts endpoint)
    {
        return endpointStateMap.containsKey(endpoint);
    }

    public int getCurrentGenerationNumber(InetAddressAndPorts endpoint)
    {
     ...
     */
    private boolean sendGossip(MessageOut<GossipDigestSyn> message, Set<InetAddressAndPorts> epSet)
    {
        List<InetAddressAndPorts> liveEndpoints = ImmutableList.copyOf(epSet);
        ...
        InetAddressAndPorts to = liveEndpoints.get(index);
        if (logger.isTraceEnabled())
    ...
    private void maybeGossipToSeed(MessageOut<GossipDigestSyn> prod)
    {
        ...
        if (size > 0)
        {
            if (size == 1 && seeds.contains(FBUtilities.getBroadcastAddressAndPorts()))
            {
                return;
            }
    ...
    public boolean isGossipOnlyMember(InetAddressAndPorts endpoint)
    {
        EndpointState epState = endpointStateMap.get(endpoint);
        if (epState == null)
        {
     ...
     */
    public boolean isSafeForStartup(InetAddressAndPorts endpoint, UUID localHostUUID, boolean isBootstrapping)
    {
        EndpointState epState = endpointStateMap.get(endpoint);
    ...
    private void doStatusCheck()
    {
        ...
        Set<InetAddressAndPorts> eps = endpointStateMap.keySet();
        for (InetAddressAndPorts endpoint : eps)
        {
        ...
        {
            for (Entry<InetAddressAndPorts, Long> entry : justRemovedEndpoints.entrySet())
            {
    ...
    protected long getExpireTimeForEndpoint(InetAddressAndPorts endpoint)
    {
    ...
    public EndpointState getEndpointStateForEndpoint(InetAddressAndPorts ep)
    {
        return endpointStateMap.get(ep);
    }

    public boolean valuesEqual(InetAddressAndPorts ep1, InetAddressAndPorts ep2, ApplicationState as)
    {
    ...
    public Set<Entry<InetAddressAndPorts, EndpointState>> getEndpointStates()
    {
        return endpointStateMap.entrySet();
    }

    public UUID getHostId(InetAddressAndPorts endpoint)
    {
        return UUID.fromString(getEndpointStateForEndpoint(endpoint).getApplicationState(ApplicationState.HOST_ID).value);
    }

    EndpointState getStateForVersionBiggerThan(InetAddressAndPorts forEndpoint, int version)
    {
     ...
     */
    public int compareEndpointStartup(InetAddressAndPorts addr1, InetAddressAndPorts addr2)
    {
    ...
    void notifyFailureDetector(Map<InetAddressAndPorts, EndpointState> remoteEpStateMap)
    {
        for (Entry<InetAddressAndPorts, EndpointState> entry : remoteEpStateMap.entrySet())
        {
            notifyFailureDetector(entry.getKey(), entry.getValue());
        }
    }

    void notifyFailureDetector(InetAddressAndPorts endpoint, EndpointState remoteEndpointState)
    {
    ...
    private void markAlive(final InetAddressAndPorts addr, final EndpointState localState)
    {
        localState.markDead();
    ...
    @VisibleForTesting
    public void realMarkAlive(final InetAddressAndPorts addr, final EndpointState localState)
    {
        if (logger.isTraceEnabled())
    ...
    @VisibleForTesting
    public void markDead(InetAddressAndPorts addr, EndpointState localState)
    {
        if (logger.isTraceEnabled())
     ...
     */
    private void handleMajorStateChange(InetAddressAndPorts ep, EndpointState epState)
    {
    ...
    public boolean isAlive(InetAddressAndPorts endpoint)
    {
    ...
    void applyStateLocally(Map<InetAddressAndPorts, EndpointState> epStateMap)
    {
        for (Entry<InetAddressAndPorts, EndpointState> entry : epStateMap.entrySet())
        {
            InetAddressAndPorts ep = entry.getKey();
            if ( ep.equals(FBUtilities.getBroadcastAddressAndPorts()) && !isInShadowRound())
                continue;
    ...
    private void applyNewStates(InetAddressAndPorts addr, EndpointState localState, EndpointState remoteState)
    {
        ...
        Set<Entry<ApplicationState, VersionedValue>> filtered = remoteStates.stream().filter(entry -> {
           ...
           {
               case INTERNAL_IP:
                    return remoteState.getApplicationState(ApplicationState.INTERNAL_ADDRESS_AND_PORTS) == null;
               case STATUS:
                   return remoteState.getApplicationState(ApplicationState.STATUS_WITH_PORTS) == null;
               case RPC_ADDRESS:
                   return remoteState.getApplicationState(ApplicationState.NATIVE_ADDRESS_AND_PORTS) == null;
               default:
                   return true;
           }
    ...
    private void doBeforeChangeNotifications(InetAddressAndPorts addr, EndpointState epState, ApplicationState apState, VersionedValue newValue)
    {
        for (IEndpointStateChangeSubscriber subscriber : subscribers)
        {
    ...
    private void doOnChangeNotifications(InetAddressAndPorts addr, ApplicationState state, VersionedValue value)
    {
        for (IEndpointStateChangeSubscriber subscriber : subscribers)
        {
    ...
    private void sendAll(GossipDigest gDigest, Map<InetAddressAndPorts, EndpointState> deltaEpStateMap, int maxRemoteVersion)
    {
    ...
    */
    void examineGossiper(List<GossipDigest> gDigestList, List<GossipDigest> deltaGossipDigestList, Map<InetAddressAndPorts, EndpointState> deltaEpStateMap)
    {
    ...
    public void start(int generationNbr, Map<ApplicationState, VersionedValue> preloadLocalStates)
    {
        ...
        EndpointState localState = endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts());
        ...
        try
        {
            while (true)
            {
                ...
                { // CASSANDRA-8072, retry at the beginning and every 5 seconds
                    ...
                    for (InetAddressAndPorts seed : seeds)
                ...
                if (slept > StorageService.RING_DELAY)
                {
                    ...
                    if (!DatabaseDescriptor.getSeeds().contains(FBUtilities.getBroadcastAddressAndPorts()))
                        throw new RuntimeException("Unable to gossip with any seeds " + DatabaseDescriptor.getSeeds() + " and " + FBUtilities.getBroadcastAddressAndPorts());
    ...
    {
        for (InetAddressAndPorts seed : DatabaseDescriptor.getSeeds())
        {
            if (seed.equals(FBUtilities.getBroadcastAddressAndPorts()))
                continue;
            seeds.add(seed);
        }
    }
    ...
    public void maybeInitializeLocalState(int generationNbr)
    {
        ...
        localState.markAlive();
        endpointStateMap.putIfAbsent(FBUtilities.getBroadcastAddressAndPorts(), localState);
    }

    public void forceNewerGeneration()
    {
        EndpointState epstate = endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts());
        epstate.getHeartBeatState().forceNewerGenerationUnsafe();
    }
     ...
     */
    public void addSavedEndpoint(InetAddressAndPorts ep)
    {
    ...
    private void addLocalApplicationStateInternal(ApplicationState state, VersionedValue value)
    {
        ...
        EndpointState epState = endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts());
        InetAddressAndPorts epAddr = FBUtilities.getBroadcastAddressAndPorts();
        assert epState != null;
    ...
    public void stop()
    {
        EndpointState mystate = endpointStateMap.get(FBUtilities.getBroadcastAddressAndPorts());
        if (mystate != null && !isSilentShutdownState(mystate) && StorageService.instance.isJoined())
        {
            logger.info("Announcing shutdown");
            addLocalApplicationState(ApplicationState.STATUS_WITH_PORTS, StorageService.instance.valueFactory.shutdown(true));
            addLocalApplicationState(ApplicationState.STATUS, StorageService.instance.valueFactory.shutdown(true));
            MessageOut message = new MessageOut(MessagingService.Verb.GOSSIP_SHUTDOWN);
            for (InetAddressAndPorts ep : liveEndpoints)
    ...
    protected void maybeFinishShadowRound(InetAddressAndPorts respondent, boolean isInShadowRound)
    {
        if (inShadowRound)
        {
            if (!isInShadowRound)
    ...
    @VisibleForTesting
    public void initializeNodeUnsafe(InetAddressAndPorts addr, UUID uuid, int generationNbr)
    {
        HeartBeatState hbState = new HeartBeatState(generationNbr);
    ...
    @VisibleForTesting
    public void injectApplicationState(InetAddressAndPorts endpoint, ApplicationState state, VersionedValue value)
    {
    ...
    {
        return getEndpointDowntime(InetAddressAndPorts.getByName(address));
    }

    public int getCurrentGenerationNumber(String address) throws UnknownHostException
    {
        return getCurrentGenerationNumber(InetAddressAndPorts.getByName(address));
    }

    public void addExpireTimeForEndpoint(InetAddressAndPorts endpoint, long expireTime)
    {
        if (logger.isDebugEnabled())
        {
    ...
    public CassandraVersion getReleaseVersion(InetAddressAndPorts ep)
    {
