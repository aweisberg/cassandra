...
public class SSTableTxnWriter extends Transactional.AbstractTransactional implements Transactional
{
    ...
    public static SSTableTxnWriter create(ColumnFamilyStore cfs, Descriptor descriptor, long keyCount, long repairedAt, TimeUUID pendingRepair, SerializationHeader header)
    {
        LifecycleTransaction txn = LifecycleTransaction.offline(OperationType.WRITE);
        SSTableMultiWriter writer = cfs.createSSTableMultiWriter(descriptor, keyCount, repairedAt, pendingRepair, header, txn);
                                                    ...
                                                    SerializationHeader header)
    {
        ...
        try
        {
            writer = new RangeAwareSSTableWriter(cfs, keyCount, repairedAt, pendingRepair, type, 0, 0, txn, header);
        }
        catch (IOException e)
        {
                                          ...
                                          SSTable.Owner owner)
    {
        ...
        LifecycleTransaction txn = LifecycleTransaction.offline(OperationType.WRITE);
        SSTableMultiWriter writer = SimpleSSTableMultiWriter.create(descriptor, keyCount, repairedAt, pendingRepair, metadata, null, 0, header, indexGroups, txn, owner);
        return new SSTableTxnWriter(txn, writer);
    }
}
