...
public class SSTableLoader implements StreamEventHandler
{
    ...
    private final Set<InetAddressAndPorts> failedHosts = new HashSet<>();

    private final List<SSTableReader> sstables = new ArrayList<>();
    private final Multimap<InetAddressAndPorts, StreamSession.SSTableStreamingSections> streamingDetails = HashMultimap.create();
    ...
    protected Collection<SSTableReader> openSSTables(final Map<InetAddressAndPorts, Collection<Range<Token>>> ranges)
    {
        ...
        LifecycleTransaction.getFiles(directory.toPath(),
                                      (file, type) ->
                                      {
                                          ...
                                          try
                                          {
                                              ...
                                              // keys per host
                                              for (Map.Entry<InetAddressAndPorts, Collection<Range<Token>>> entry : ranges.entrySet())
                                              {
    ...
    {
        return stream(Collections.<InetAddressAndPorts>emptySet());
    }

    public StreamResultFuture stream(Set<InetAddressAndPorts> toIgnore, StreamEventHandler... listeners)
    {
        ...
        Map<InetAddressAndPorts, Collection<Range<Token>>> endpointToRanges = client.getEndpointToRangesMap();
        ...
        for (Map.Entry<InetAddressAndPorts, Collection<Range<Token>>> entry : endpointToRanges.entrySet())
        {
            InetAddressAndPorts remote = entry.getKey();
    ...
    public Set<InetAddressAndPorts> getFailedHosts()
    {
        return failedHosts;
    }
    ...
    {
        private final Map<InetAddressAndPorts, Collection<Range<Token>>> endpointToRanges = new HashMap<>();
        ...
        public Map<InetAddressAndPorts, Collection<Range<Token>>> getEndpointToRangesMap()
        {
            return endpointToRanges;
        }

        protected void addRangeForEndpoint(Range<Token> range, InetAddressAndPorts endpoint)
        {
