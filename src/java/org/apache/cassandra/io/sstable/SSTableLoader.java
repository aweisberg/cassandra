...
public class SSTableLoader implements StreamEventHandler
{
    ...
    private final Set<Endpoint> failedHosts = new HashSet<>();

    private final List<SSTableReader> sstables = new ArrayList<>();
    private final Multimap<Endpoint, OutgoingStream> streamingDetails = HashMultimap.create();
    ...
    protected Collection<SSTableReader> openSSTables(final Map<Endpoint, Collection<Range<Token>>> ranges)
    {
        ...
        LifecycleTransaction.getFiles(directory.toPath(),
                                      (file, type) ->
                                      {
                                          ...
                                          try
                                          {
                                              ...
                                              // keys per host
                                              for (Map.Entry<Endpoint, Collection<Range<Token>>> entry : ranges.entrySet())
                                              {
    ...
    {
        return stream(Collections.<Endpoint>emptySet());
    }

    public StreamResultFuture stream(Set<Endpoint> toIgnore, StreamEventHandler... listeners)
    {
        ...
        Map<Endpoint, Collection<Range<Token>>> endpointToRanges = client.getEndpointToRangesMap();
        ...
        for (Map.Entry<Endpoint, Collection<Range<Token>>> entry : endpointToRanges.entrySet())
        {
            Endpoint remote = entry.getKey();
    ...
    public Set<Endpoint> getFailedHosts()
    {
        return failedHosts;
    }
    ...
    {
        private final Map<Endpoint, Collection<Range<Token>>> endpointToRanges = new HashMap<>();
        ...
        public Map<Endpoint, Collection<Range<Token>>> getEndpointToRangesMap()
        {
            return endpointToRanges;
        }

        protected void addRangeForEndpoint(Range<Token> range, Endpoint endpoint)
        {
