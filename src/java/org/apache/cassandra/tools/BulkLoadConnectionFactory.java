...
public class BulkLoadConnectionFactory implements StreamConnectionFactory
{
    ...
    public Socket createConnection(InetAddressAndPorts peer) throws IOException
    {
        ...
        {
            if (outboundBindAny)
                return SSLFactory.getSocket(encryptionOptions, peer.address, peer.sslport);
            else
                return SSLFactory.getSocket(encryptionOptions, peer.address, peer.sslport, FBUtilities.getJustLocalAddress(), 0);
        }
        else
        {
