...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    public ReplicaSet getLocalReplicas(String keyspaceName)
    {
        return getReplicasForEndpoint(keyspaceName, FBUtilities.getBroadcastAddressAndPort());
    }

    public ReplicaSet getLocalFullRanges(String keyspaceName)
    {
        return getReplicasForEndpoint(keyspaceName, FBUtilities.getBroadcastAddressAndPort()).filter(Replica::isFull);
    }
        ...
        try
        {
            ...
            if (keyspace == null)
            {
                for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces())
                    streamer.addRanges(keyspaceName, getLocalFullRanges(keyspaceName));
            }
            else if (tokens == null)
            {
                streamer.addRanges(keyspace, getLocalFullRanges(keyspace));
            }
            else
            {
                ...
                ReplicaSet localFullRanges = getLocalFullRanges(keyspace);
                ReplicaList streamRanges = new ReplicaList(ranges.size());
                for (Range<Token> specifiedRange : ranges)
                {
                    boolean foundParentRange = false;
                    for (Replica localRange : localFullRanges)
                    {
    ...
    private Map<List<String>, List<String>> getRangeToNativeaddressMap(String keyspace, boolean withPort)
    {
        ...
        for (Map.Entry<Range<Token>, ReplicaList> entry : getRangeToAddressMap(keyspace).entrySet())
        {
            ...
            {
                rpcaddrs.add(getNativeaddress(replicas.getEndpoint(), withPort));
            }
    ...
    private Map<List<String>, List<String>> getPendingRangeToEndpointMap(String keyspace, boolean withPort)
    {
        ...
        Map<List<String>, List<String>> map = new HashMap<>();
        for (Map.Entry<Range<Token>, ReplicaSet> entry : tokenMetadata.getPendingRangesMM(keyspace).asMap().entrySet())
        {
            map.put(entry.getKey().asList(), Replicas.stringify(entry.getValue(), withPort));
        }
        return map;
    ...
    private void restoreReplicaCount(InetAddressAndPort endpoint, final InetAddressAndPort notifyEndpoint)
    {
        ...
        for (String keyspaceName : replicasToFetch.keySet())
        {
            for (Map.Entry<InetAddressAndPort, ReplicaSet> entry : replicasToFetch.get(keyspaceName))
            {
                ...
                //We only want to fetch the full ranges and not transient ranges
                //Transient data we aren't fetching will propagate via the next repair to where it belongs
                //We don't need to increase replication of it right now
                //We definitely don't want to fetch data we transiently replicate from full replicas
                //As that will trigger a lot of extra work in the next repair
                ReplicaSet fullRanges = replicas.filter(Replica::isFull);
                if (logger.isDebugEnabled())
    ...
    public Collection<Range<Token>> getPrimaryRangeForEndpointWithinDC(String keyspace, InetAddressAndPort referenceEndpoint)
    {
        ...
        for (Token token : metadata.sortedTokens())
        {
            ...
            for (Replica replica : replicas)
            {
                ...
                {
                    if (replica.getEndpoint().equals(referenceEndpoint))
                    {
    ...
    */
    ...
    private UUID getPreferredHintsStreamTarget()
    {
        ...
        else
        {
            ...
            InetAddressAndPort hintsDestinationHost = candidates.get(0).getEndpoint();
    ...
    private LinkedHashMap<InetAddressAndPort, Float> getEffectiveOwnership(String keyspace)
    {
        ...
        for (Collection<InetAddressAndPort> endpoints : endpointsGroupedByDc)
        {
            ...
            for (InetAddressAndPort endpoint : endpoints)
            {
                ...
                {
                    if (tokenOwnership.containsKey(replica.getRange().right))
                        ownership += tokenOwnership.get(replica.getRange().right);
                }
    ...
    private Future<StreamState> streamRanges(Map<String, ReplicaMultimap<Range<Token>, ReplicaSet>> rangesToStreamByKeyspace)
    {
        ...
        for (Map.Entry<String, ReplicaMultimap<Range<Token>, ReplicaSet>> entry : rangesToStreamByKeyspace.entrySet())
        {
            ...
            for (Map.Entry<Range<Token>, Replica> endPointEntry : rangesWithEndpoints.entries())
            {
                ...
                // Don't stream data to transient replicas at all
                // Only stream to full replicas. Since we are streaming to all full replicas
                // this is a synchronizing action similar to repair so there
                // is no point in moving data to transient replicas that will later be moved
                // to the full replicas. This is predicated on the assumption that when
                // we do this we are always streaming to all full replicas and none are omitted
                // or down an fail to have the data streamed for whatever reason.
                //TODO dtest this change
