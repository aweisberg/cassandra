...
public abstract class TxnQuery implements Query
{
    ...
    public static final TxnQuery RANGE_QUERY = new TxnQuery()
    {
        ...
        private Result concat(TxnData data, Read read)
        {
            ...
            value.sort();
//            PartitionIterator result = PartitionIterators.concat(Lists.transform(value.isEmpty() ? value : value.subList(0, Math.min(2, value.size())), v -> PartitionIterators.singletonIterator(v.rowIterator(command.isReversed()))));
            PartitionIterator result = PartitionIterators.concat(Lists.transform(value, v -> PartitionIterators.singletonIterator(v.rowIterator(command.isReversed()))));
//            // Need to repeat enforcing the limit
//            // The data has already been filtered so don't need to provide nowInSeconds
//            result = command.limits().filter(result, Long.MIN_VALUE);
            return new TxnRangeReadResult(result);
        }
    };
