...
public class TxnRangeRead extends AbstractSerialized<ReadCommand> implements TxnRead
{
    ...
    private AsyncChain<Data> performLocalRead(PartitionRangeReadCommand command, Range r, int nowInSeconds)
    {
        Callable<Data> readCallable = () ->
        {
            ...
            // TODO (review): Really winging it here with max vs min key bound, and the distinction between token and key bound
            // as it pertains to how you request the query to execute. It seems like in the range query the actual left and right bounds of the whole query
            // are minKeyBound and then if it is split everything in the splits is maxKeyBound except for the leftmost and rightmost across all the splits.
            // So by preserving the startKey and stopKey from the command using those we get the correct min/max key boundness otherwise it's set to max
            ...
            // TODO (required): Does this handle range continuation correctly? I think it does because while we further split the range
            // the continuation will be the left most one and then for subsequent ranges we want to start at the beginning of the range
            PartitionRangeReadCommand read = command.forSubRangeWithNowInSeconds(nowInSeconds, subRange, startTokenKey.equals(r.start()));

            // TODO (required): Since we are splitting these up into separate range reads we might return too much results and need to apply the limits
            // again after concatenating them in `TxnQuery`
                 ...
                 PartitionIterator iterator = UnfilteredPartitionIterators.filter(partition, read.nowInSec()))
            {
                ...
                while(iterator.hasNext())
                {
                    try (RowIterator rows = iterator.next())
                    {
                        ...
                        // TODO (review): does `selectsFullPartition` also apply to range reads?
                        if (filtered.hasRows() || read.selectsFullPartition())
                        {
                            logger.info("Had FilteredPartition with token " + filtered.partitionKey().getToken());
                            value.add(filtered);
                        }
                    }
                }
    ...
    {
        return new TxnRangeRead(bytes(), cassandraConsistencyLevel, covering.slice(ranges));
    }
