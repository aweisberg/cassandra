...
public class AccordRepair
{
    ...
    private Pair<List<accord.primitives.Range>, Long> repairRange(TokenRange range) throws Throwable
    {
        ...
        try
        {
            ...
            logger.info("Starting Accord repair of intersecting ranges " + ranges);
            long maxHlc = AccordService.getBlocking(service.maxConflict(ranges).flatMap(conflict -> {
                ...
                return service.sync("[repairId #" + repairId + ']', conflictMax, Ranges.of(range), ids, NoLocal, syncRemote).map(ignored -> conflictMax.hlc());
            }), null, ranges, bookkeeping, start, start + getAccordRepairTimeoutNanos(), false);
            logger.info(format("Accord repair took %.3f", sw.elapsed(TimeUnit.MILLISECONDS) / 1000.0));
            waiting = null;
