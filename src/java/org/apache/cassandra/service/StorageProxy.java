...
public class StorageProxy implements StorageProxyMBean
{
    ...
    private static final CacheLoader<Endpoint, AtomicInteger> hintsInProgress = new CacheLoader<Endpoint, AtomicInteger>()
    {
        public AtomicInteger load(Endpoint inetAddress)
        {
    ...
    throws UnavailableException, IsBootstrappingException, RequestFailureException, RequestTimeoutException, InvalidRequestException
    {
        ...
        try
        {
            ...
            while (System.nanoTime() - queryStartNanoTime < timeout)
            {
                ...
                List<Endpoint> liveEndpoints = p.liveEndpoints;
    ...
    private static Predicate<Endpoint> sameDCPredicateFor(final String dc)
    {
        final IEndpointSnitch snitch = DatabaseDescriptor.getEndpointSnitch();
        return new Predicate<Endpoint>()
        {
            public boolean apply(Endpoint host)
            {
    ...
    private static PaxosParticipants getPaxosParticipants(TableMetadata metadata, DecoratedKey key, ConsistencyLevel consistencyForPaxos) throws UnavailableException
    {
        ...
        List<Endpoint> naturalEndpoints = StorageService.instance.getNaturalEndpoints(metadata.keyspace, tk);
        Collection<Endpoint> pendingEndpoints = StorageService.instance.getTokenMetadata().pendingEndpointsFor(tk, metadata.keyspace);
        if (consistencyForPaxos == ConsistencyLevel.LOCAL_SERIAL)
        {
            ...
            Predicate<Endpoint> isLocalDc = sameDCPredicateFor(localDc);
            naturalEndpoints = ImmutableList.copyOf(Iterables.filter(naturalEndpoints, isLocalDc));
            pendingEndpoints = ImmutableList.copyOf(Iterables.filter(pendingEndpoints, isLocalDc));
        }
        int participants = pendingEndpoints.size() + naturalEndpoints.size();
        int requiredParticipants = participants / 2 + 1; // See CASSANDRA-8346, CASSANDRA-833
        List<Endpoint> liveEndpoints = ImmutableList.copyOf(Iterables.filter(Iterables.concat(naturalEndpoints, pendingEndpoints), IAsyncCallback.isAlive));
    ...
    throws WriteTimeoutException, WriteFailureException
    {
        ...
        while (System.nanoTime() - queryStartNanoTime < timeout)
        {
            ...
            Iterable<Endpoint> missingMRC = summary.replicasMissingMostRecentCommit(metadata, nowInSec);
     ...
     */
    private static void sendCommit(Commit commit, Iterable<Endpoint> replicas)
    {
        MessageOut<Commit> message = new MessageOut<Commit>(MessagingService.Verb.PAXOS_COMMIT, commit, Commit.serializer);
        for (Endpoint target : replicas)
            MessagingService.instance().sendOneWay(message, target);
    }

    private static PrepareCallback preparePaxos(Commit toPrepare, List<Endpoint> endpoints, int requiredParticipants, ConsistencyLevel consistencyForPaxos, long queryStartNanoTime)
    ...
    private static boolean proposePaxos(Commit proposal, List<Endpoint> endpoints, int requiredParticipants, boolean timeoutIfPartial, ConsistencyLevel consistencyLevel, long queryStartNanoTime)
    throws WriteTimeoutException
    {
    ...
    private static void commitPaxos(Commit proposal, ConsistencyLevel consistencyLevel, boolean allowHints, long queryStartNanoTime) throws WriteTimeoutException
    {
        ...
        List<Endpoint> naturalEndpoints = StorageService.instance.getNaturalEndpoints(keyspace.getName(), tk);
        Collection<Endpoint> pendingEndpoints = StorageService.instance.getTokenMetadata().pendingEndpointsFor(tk, keyspace.getName());
        ...
        for (Endpoint destination : Iterables.concat(naturalEndpoints, pendingEndpoints))
        {
    ...
    private static void hintMutation(Mutation mutation)
    {
        ...
        Iterable<Endpoint> endpoints = StorageService.instance.getNaturalAndPendingEndpoints(keyspaceName, token);
        ArrayList<Endpoint> endpointsToHint = new ArrayList<>(Iterables.size(endpoints));
    ...
    throws UnavailableException, OverloadedException, WriteTimeoutException
    {
        ...
        try
        {
            ...
            else
            {
                ...
                final Collection<Endpoint> batchlogEndpoints = Collections.singleton(FBUtilities.getBroadcastAddressAndPort());
                ...
                for (Mutation mutation : mutations)
                {
                    ...
                    Optional<Endpoint> pairedEndpoint = ViewUtils.getViewNaturalEndpoint(keyspaceName, baseToken, tk);
                    Collection<Endpoint> pendingEndpoints = StorageService.instance.getTokenMetadata().pendingEndpointsFor(tk, keyspaceName);
    ...
    throws UnavailableException, OverloadedException, WriteTimeoutException
    {
        ...
        try
        {
            ...
            final Collection<Endpoint> batchlogEndpoints = getBatchlogEndpoints(localDataCenter, batchConsistencyLevel);
            final UUID batchUUID = UUIDGen.getTimeUUID();
    ...
    public static boolean canDoLocalRequest(Endpoint replica)
    {
    ...
    private static void syncWriteToBatchlog(Collection<Mutation> mutations, Collection<Endpoint> endpoints, UUID uuid, long queryStartNanoTime)
    throws WriteTimeoutException, WriteFailureException
    {
    ...
    private static void asyncRemoveFromBatchlog(Collection<Endpoint> endpoints, UUID uuid)
    {
    ...
    private static void asyncWriteBatchedMutations(List<WriteResponseHandlerWrapper> wrappers, String localDataCenter, Stage stage)
    {
        ...
        {
            Iterable<Endpoint> endpoints = Iterables.concat(wrapper.handler.naturalEndpoints, wrapper.handler.pendingEndpoints);
    ...
    throws UnavailableException, OverloadedException
    {
        ...
        Token tk = mutation.key().getToken();
        List<Endpoint> naturalEndpoints = StorageService.instance.getNaturalEndpoints(keyspaceName, tk);
        Collection<Endpoint> pendingEndpoints = StorageService.instance.getTokenMetadata().pendingEndpointsFor(tk, keyspaceName);
    ...
    private static WriteResponseHandlerWrapper wrapViewBatchResponseHandler(Mutation mutation,
                                                                            ConsistencyLevel consistency_level,
                                                                            ConsistencyLevel batchConsistencyLevel,
                                                                            List<Endpoint> naturalEndpoints,
     ...
     */
    private static Collection<Endpoint> getBatchlogEndpoints(String localDataCenter, ConsistencyLevel consistencyLevel)
    throws UnavailableException
    {
        TokenMetadata.Topology topology = StorageService.instance.getTokenMetadata().cachedOnlyTokenMap().getTopology();
        Multimap<String, Endpoint> localEndpoints = HashMultimap.create(topology.getDatacenterRacks().get(localDataCenter));
        String localRack = DatabaseDescriptor.getEndpointSnitch().getRack(FBUtilities.getBroadcastAddressAndPort());

        Collection<Endpoint> chosenEndpoints = new BatchlogManager.EndpointFilter(localRack, localEndpoints).filter();
    ...
    throws OverloadedException
    {
        ...
        // this dc replicas:
        Collection<Endpoint> localDc = null;
        // extra-datacenter replicas, grouped by dc
        Map<String, Collection<Endpoint>> dcGroups = null;
        // only need to create a Message for non-local writes
        MessageOut<Mutation> message = null;

        boolean insertLocal = false;
        ArrayList<Endpoint> endpointsToHint = null;

        List<Endpoint> backPressureHosts = null;

        for (Endpoint destination : targets)
        {
            checkHintOverload(destination);
            ...
            if (FailureDetector.instance.isAlive(destination))
            {
                ...
                else
                {
                    ...
                    {
                        Collection<Endpoint> messages = (dcGroups != null) ? dcGroups.get(dc) : null;
                        if (messages == null)
                        {
        ...
        if (localDc != null)
        {
            for (Endpoint destination : localDc)
                MessagingService.instance().sendRR(message, destination, responseHandler, true);
        }
        if (dcGroups != null)
        {
            // for each datacenter, send the message to one node to relay the write to other replicas
            for (Collection<Endpoint> dcTargets : dcGroups.values())
                sendMessagesToNonlocalDC(message, dcTargets, responseHandler);
        }
    }

    private static void checkHintOverload(Endpoint destination)
    {
    ...
    private static void sendMessagesToNonlocalDC(MessageOut<? extends IMutation> message,
                                                 Collection<Endpoint> targets,
                                                 AbstractWriteResponseHandler<IMutation> handler)
    {
        Iterator<Endpoint> iter = targets.iterator();
        int[] messageIds = new int[targets.size()];
        Endpoint target = iter.next();

        int idIdx = 0;
        // Add the other destinations of the same message as a FORWARD_HEADER entry
        while (iter.hasNext())
        {
            Endpoint destination = iter.next();
    ...
    {
        Endpoint endpoint = findSuitableEndpoint(cm.getKeyspaceName(), cm.key(), localDataCenter, cm.consistency());
     ...
     */
    private static Endpoint findSuitableEndpoint(String keyspaceName, DecoratedKey key, String localDataCenter, ConsistencyLevel cl) throws UnavailableException
    {
        ...
        List<Endpoint> localEndpoints = new ArrayList<>(endpoints.size());
    ...
    private static Runnable counterWriteTask(final IMutation mutation,
                                             final Iterable<Endpoint> targets,
                                             ...
                                             final String localDataCenter)
    {
        return new DroppableRunnable(MessagingService.Verb.COUNTER_MUTATION)
        {
            ...
            public void runMayThrow() throws OverloadedException, WriteTimeoutException
            {
                ...
                Set<Endpoint> remotes = Sets.difference(ImmutableSet.copyOf(targets),
    ...
    public static List<Endpoint> getLiveSortedEndpoints(Keyspace keyspace, ByteBuffer key)
    {
        return getLiveSortedEndpoints(keyspace, StorageService.instance.getTokenMetadata().decorateKey(key));
    }

    public static List<Endpoint> getLiveSortedEndpoints(Keyspace keyspace, RingPosition pos)
    {
        List<Endpoint> liveEndpoints = StorageService.instance.getLiveNaturalEndpoints(keyspace, pos);
        DatabaseDescriptor.getEndpointSnitch().sortByProximity(FBUtilities.getBroadcastAddressAndPort(), liveEndpoints);
        return liveEndpoints;
    }

    private static List<Endpoint> intersection(List<Endpoint> l1, List<Endpoint> l2)
    {
        ...
        List<Endpoint> inter = new ArrayList<>(l1);
        inter.retainAll(l2);
        return inter;
    }
    ...
    private static class RangeForQuery
    {
        ...
        public final List<Endpoint> liveEndpoints;
        public final List<Endpoint> filteredEndpoints;

        public RangeForQuery(AbstractBounds<PartitionPosition> range, List<Endpoint> liveEndpoints, List<Endpoint> filteredEndpoints)
        {
            this.range = range;
    ...
    private static class RangeIterator extends AbstractIterator<RangeForQuery>
    {
        ...
        protected RangeForQuery computeNext()
        {
            ...
            List<Endpoint> liveEndpoints = getLiveSortedEndpoints(keyspace, range.right);
    ...
    private static class RangeMerger extends AbstractIterator<RangeForQuery>
    {
        ...
        protected RangeForQuery computeNext()
        {
            ...
            while (ranges.hasNext())
            {
                ...
                List<Endpoint> merged = intersection(current.liveEndpoints, next.liveEndpoints);

                // Check if there is enough endpoint for the merge to be possible.
                if (!consistency.isSufficientLiveNodes(keyspace, merged))
                    break;

                List<Endpoint> filteredMerged = consistency.filterForQuery(keyspace, merged);
    ...
    private static class RangeCommandIterator extends AbstractIterator<RowIterator> implements PartitionIterator
    {
        ...
        private SingleRangeResponse query(RangeForQuery toQuery, boolean isFirst)
        {
            ...
            List<Endpoint> minimalEndpoints = toQuery.filteredEndpoints.subList(0, minResponses);
            ...
            {
                for (Endpoint endpoint : toQuery.filteredEndpoints)
                {
    ...
    public static Map<String, List<String>> describeSchemaVersions(boolean withPort)
    {
        ...
        final Map<Endpoint, UUID> versions = new ConcurrentHashMap<>();
        final Set<Endpoint> liveHosts = Gossiper.instance.getLiveMembers();
        ...
        for (Endpoint endpoint : liveHosts)
    ...
    public static boolean shouldHint(Endpoint ep)
    {
    ...
    public static void truncateBlocking(String keyspace, String cfname) throws UnavailableException, TimeoutException
    {
        ...
        Set<Endpoint> allEndpoints = StorageService.instance.getLiveRingMembers(true);
    ...
    {
        public final Collection<Endpoint> targets;

        protected HintRunnable(Collection<Endpoint> targets)
        {
    ...
    private static AtomicInteger getHintsInProgressFor(Endpoint destination)
    {
        try
        {
    ...
    public static Future<Void> submitHint(Mutation mutation, Endpoint target, AbstractWriteResponseHandler<IMutation> responseHandler)
    {
                                          ...
                                          AbstractWriteResponseHandler<IMutation> responseHandler)
    {
        HintRunnable runnable = new HintRunnable(targets)
        {
            public void runMayThrow()
            {
                Set<Endpoint> validTargets = new HashSet<>(targets.size());
    ...
    private static Future<Void> submitHint(HintRunnable runnable)
    {
        ...
        for (Endpoint target : runnable.targets)
    ...
    {
        final List<Endpoint> liveEndpoints;
        final int participants;

        PaxosParticipants(List<Endpoint> liveEndpoints, int participants)
        {
            this.liveEndpoints = liveEndpoints;
