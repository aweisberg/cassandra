...
public abstract class AbstractReadExecutor
{
    ...
    private void makeRequests(ReadCommand readCommand, ReplicaCollection replicas)
    {
        ...
        for (Replica replica: replicas)
        {
            ...
            if (traceState != null)
                traceState.trace("reading {} from {} replica {}", readCommand.isDigestQuery() ? "digest" : "data", replica.isFull() ? "full" : "transient", endpoint);
            logger.trace("reading {} from {} replica {}", readCommand.isDigestQuery() ? "digest" : "data", replica.isFull() ? "full" : "transient", endpoint);
    ...
    public void awaitResponses() throws ReadTimeoutException
    {
        ...
        if (digestResolver.responsesMatch())
        {
            ...
            // TODO: does that mean that we would not repair in both cases, even if transient is a coordinator?
        }
        else
        {
