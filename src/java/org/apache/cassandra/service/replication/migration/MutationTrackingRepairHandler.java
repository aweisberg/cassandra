        ...
        new FutureCallback<>()
        {
            ...
            public void onSuccess(RepairResult repairResult)
            {
                try
                {
                    ...
                    {
                        logger.debug("Repair session {} (parent session {}) completed for {}.{} but the table has no ranges left to migrate, not advancing mutation tracking migration",
                                     desc.sessionId, desc.parentSessionId, keyspace, tableName);
                        return;
                    }
                    ...
                    {
                        logger.debug("Repair session {} (parent session {}) completed for {}.{} but is ineligible to advance mutation tracking migration because {}",
                    ...
                    {
                        logger.debug("Repair session {} (parent session {}) completed for {}.{} but the repair started at epoch {}, before the migration started at epoch {}, not advancing mutation tracking migration",
