...
public class TokenAllocation
{
                                                                     ...
                                                                     StrategyAdapter strategy, Collection<Token> tokens)
    {
        ...
        for (Token t : tokens)
        {
            ...
            {
                Endpoint other = tokenMetadata.getEndpoint(t);
    ...
    public static Map<Endpoint, Double> evaluateReplicatedOwnership(TokenMetadata tokenMetadata, AbstractReplicationStrategy rs)
    {
        Map<Endpoint, Double> ownership = Maps.newHashMap();
    ...
    static void addOwnership(final TokenMetadata tokenMetadata, final AbstractReplicationStrategy rs, Token current, Token next, Map<Endpoint, Double> ownership)
    {
        double size = current.size(next);
        Token representative = current.getPartitioner().midpoint(current, next);
        for (Endpoint n : rs.calculateNaturalEndpoints(representative, tokenMetadata))
        {
    ...
    public static SummaryStatistics replicatedOwnershipStats(TokenMetadata tokenMetadata,
                                                             AbstractReplicationStrategy rs, Endpoint endpoint)
    {
        SummaryStatistics stat = new SummaryStatistics();
        StrategyAdapter strategy = getStrategy(tokenMetadata, rs, endpoint);
        for (Map.Entry<Endpoint, Double> en : evaluateReplicatedOwnership(tokenMetadata, rs).entrySet())
        {
    ...
    static TokenAllocator<Endpoint> create(TokenMetadata tokenMetadata, StrategyAdapter strategy)
    {
        NavigableMap<Token, Endpoint> sortedTokens = new TreeMap<>();
        for (Map.Entry<Token, Endpoint> en : tokenMetadata.getNormalAndBootstrappingTokenToEndpointMap().entrySet())
        {
    ...
    interface StrategyAdapter extends ReplicationStrategy<Endpoint>
    {
        ...
        boolean inAllocationRing(Endpoint other);
    }

    static StrategyAdapter getStrategy(final TokenMetadata tokenMetadata, final AbstractReplicationStrategy rs, final Endpoint endpoint)
    {
    ...
    static StrategyAdapter getStrategy(final TokenMetadata tokenMetadata, final SimpleStrategy rs, final Endpoint endpoint)
    {
        ...
        return new StrategyAdapter()
        {
            ...
            @Override
            public Object getGroup(Endpoint unit)
            {
                return unit;
            }

            @Override
            public boolean inAllocationRing(Endpoint other)
            {
                return true;
            }
        };
    ...
    static StrategyAdapter getStrategy(final TokenMetadata tokenMetadata, final NetworkTopologyStrategy rs, final IEndpointSnitch snitch, final Endpoint endpoint)
    {
