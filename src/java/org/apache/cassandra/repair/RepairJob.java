...
public class RepairJob extends AbstractFuture<RepairResult> implements Runnable
{
    ...
    public void run()
    {
        List<InetAddressAndPorts> allEndpoints = new ArrayList<>(session.endpoints);
        ...
        if (parallelismDegree != RepairParallelism.PARALLEL)
        {
            ...
            List<ListenableFuture<InetAddressAndPorts>> snapshotTasks = new ArrayList<>(allEndpoints.size());
            ...
            ListenableFuture<List<InetAddressAndPorts>> allSnapshotTasks = Futures.allAsList(snapshotTasks);
            validations = Futures.transform(allSnapshotTasks, new AsyncFunction<List<InetAddressAndPorts>, List<TreeResponse>>()
            {
                public ListenableFuture<List<TreeResponse>> apply(List<InetAddressAndPorts> endpoints)
                {
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendValidationRequest(Collection<InetAddressAndPorts> endpoints)
    {
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendSequentialValidationRequest(Collection<InetAddressAndPorts> endpoints)
    {
        String message = String.format("Requesting merkle trees for %s (to %s)", desc.columnFamily, endpoints);
        logger.info("[repair #{}] {}", desc.sessionId, message);
        Tracing.traceRepair(message);
        ...
        Queue<InetAddressAndPorts> requests = new LinkedList<>(endpoints);
        InetAddressAndPorts address = requests.poll();
        ...
        {
            final InetAddressAndPorts nextAddress = requests.poll();
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendDCAwareValidationRequest(Collection<InetAddressAndPorts> endpoints)
    {
        String message = String.format("Requesting merkle trees for %s (to %s)", desc.columnFamily, endpoints);
        logger.info("[repair #{}] {}", desc.sessionId, message);
        Tracing.traceRepair(message);
        ...
        Map<String, Queue<InetAddressAndPorts>> requestsByDatacenter = new HashMap<>();
        for (InetAddressAndPorts endpoint : endpoints)
        {
            String dc = DatabaseDescriptor.getEndpointSnitch().getDatacenter(endpoint);
            Queue<InetAddressAndPorts> queue = requestsByDatacenter.get(dc);
            if (queue == null)
            {
        ...
        for (Map.Entry<String, Queue<InetAddressAndPorts>> entry : requestsByDatacenter.entrySet())
        {
            Queue<InetAddressAndPorts> requests = entry.getValue();
            InetAddressAndPorts address = requests.poll();
            ValidationTask firstTask = new ValidationTask(desc, address, gcBefore);
            logger.info("Validating {}", address);
            session.waitForValidation(Pair.create(desc, address), firstTask);
