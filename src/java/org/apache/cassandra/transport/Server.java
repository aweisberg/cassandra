...
public class Server implements CassandraDaemon.Server
{
    ...
    private static class EventNotifier extends SchemaChangeListener implements IEndpointLifecycleSubscriber
    {
        ...
        private final Map<Endpoint, LatestEvent> latestEvents = new ConcurrentHashMap<>();
        ...
        private final Set<Endpoint> endpointsPendingJoinedNotification = ConcurrentHashMap.newKeySet();
        ...
        private Endpoint getNativeAddress(Endpoint endpoint)
        {
            try
            {
                return Endpoint.getByName(StorageService.instance.getNativeaddress(endpoint, true));
            }
            catch (UnknownHostException e)
            {
                ...
                return Endpoint.getByAddressOverrideDefaults(endpoint.address, DatabaseDescriptor.getNativeTransportPort());
            }
        }

        private void send(Endpoint endpoint, Event.NodeEvent event)
        {
        ...
        public void onJoinCluster(Endpoint endpoint)
        {
        ...
        public void onDown(Endpoint endpoint)
        {
            onStatusChange(endpoint, Event.StatusChange.nodeDown(getNativeAddress(endpoint)));
        }

        private void onTopologyChange(Endpoint endpoint, Event.TopologyChange event)
        {
            if (logger.isTraceEnabled())
        ...
        private void onStatusChange(Endpoint endpoint, Event.StatusChange event)
        {
            if (logger.isTraceEnabled())
