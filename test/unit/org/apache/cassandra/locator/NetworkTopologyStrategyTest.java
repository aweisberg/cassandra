...
public class NetworkTopologyStrategyTest
{
    ...
    public void testProperties() throws IOException, ConfigurationException
    {
        ...
        ArrayList<InetAddressAndPorts> endpoints = strategy.getNaturalEndpoints(new StringToken("123"));
    ...
    public void testLargeCluster() throws UnknownHostException, ConfigurationException
    {
        ...
        TokenMetadata metadata = new TokenMetadata();
        Map<String, String> configOptions = new HashMap<String, String>();
        Multimap<InetAddressAndPorts, Token> tokens = HashMultimap.create();
        ...
        for (int dc = 0; dc < dcRacks.length; ++dc)
        {
            ...
            for (int rack = 0; rack < dcRacks[dc]; ++rack)
            {
                for (int ep = 1; ep <= dcEndpoints[dc]/dcRacks[dc]; ++ep)
                {
                    ...
                    InetAddressAndPorts address = InetAddressAndPorts.getByAddress(ipBytes);
        ...
        {
            List<InetAddressAndPorts> endpoints = strategy.calculateNaturalEndpoints(new StringToken(testToken), metadata);
            Set<InetAddressAndPorts> epSet = new HashSet<>(endpoints);
    ...
    public void tokenFactory(TokenMetadata metadata, String token, byte[] bytes) throws UnknownHostException
    {
        ...
        InetAddressAndPorts add1 = InetAddressAndPorts.getByAddress(bytes);
    ...
    public void testCalculateEndpoints() throws UnknownHostException
    {
        ...
        List<InetAddressAndPorts> nodes = new ArrayList<>(NODES);
        for (byte i=0; i<NODES; ++i)
            nodes.add(InetAddressAndPorts.getByAddress(new byte[]{127, 0, 0, i}));
    ...
    void testEquivalence(TokenMetadata tokenMetadata, IEndpointSnitch snitch, Map<String, Integer> datacenters, Random rand)
    {
        ...
        for (int i=0; i<1000; ++i)
        {
            ...
            List<InetAddressAndPorts> expected = calculateNaturalEndpoints(token, tokenMetadata, datacenters, snitch);
            List<InetAddressAndPorts> actual = nts.calculateNaturalEndpoints(token, tokenMetadata);
    ...
    private boolean endpointsDiffer(List<InetAddressAndPorts> ep1, List<InetAddressAndPorts> ep2)
    {
        ...
        if (!ep1.get(0).equals(ep2.get(0)))
            return true;
        Set<InetAddressAndPorts> s1 = new HashSet<>(ep1);
        Set<InetAddressAndPorts> s2 = new HashSet<>(ep2);
        return !s1.equals(s2);
    }

    IEndpointSnitch generateSnitch(Map<String, Integer> datacenters, Collection<InetAddressAndPorts> nodes, Random rand)
    {
        final Map<InetAddressAndPorts, String> nodeToRack = new HashMap<>();
        final Map<InetAddressAndPorts, String> nodeToDC = new HashMap<>();
        ...
        for (InetAddressAndPorts node : nodes)
        {
    ...
    public static List<InetAddressAndPorts> calculateNaturalEndpoints(Token searchToken, TokenMetadata tokenMetadata, Map<String, Integer> datacenters, IEndpointSnitch snitch)
    {
        // we want to preserve insertion order so that the first added endpoint becomes primary
        Set<InetAddressAndPorts> replicas = new LinkedHashSet<>();
        // replicas we have found in each DC
        Map<String, Set<InetAddressAndPorts>> dcReplicas = new HashMap<>(datacenters.size());
        for (Map.Entry<String, Integer> dc : datacenters.entrySet())
            dcReplicas.put(dc.getKey(), new HashSet<InetAddressAndPorts>(dc.getValue()));

        Topology topology = tokenMetadata.getTopology();
        // all endpoints in each DC, so we can check when we have exhausted all the members of a DC
        Multimap<String, InetAddressAndPorts> allEndpoints = topology.getDatacenterEndpoints();
        // all racks in a DC so we can check when we have exhausted all racks in a DC
        Map<String, Multimap<String, InetAddressAndPorts>> racks = topology.getDatacenterRacks();
        ...
        Map<String, Set<InetAddressAndPorts>> skippedDcEndpoints = new HashMap<>(datacenters.size());
        for (Map.Entry<String, Integer> dc : datacenters.entrySet())
            skippedDcEndpoints.put(dc.getKey(), new LinkedHashSet<InetAddressAndPorts>());

        Iterator<Token> tokenIter = TokenMetadata.ringIterator(tokenMetadata.sortedTokens(), searchToken, false);
        while (tokenIter.hasNext() && !hasSufficientReplicas(dcReplicas, allEndpoints, datacenters))
        {
            Token next = tokenIter.next();
            InetAddressAndPorts ep = tokenMetadata.getEndpoint(next);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (seenRacks.get(dc).size() == racks.get(dc).keySet().size())
                    {
                        Iterator<InetAddressAndPorts> skippedIt = skippedDcEndpoints.get(dc).iterator();
                        while (skippedIt.hasNext() && !hasSufficientReplicas(dc, dcReplicas, allEndpoints, datacenters))
                        {
                            InetAddressAndPorts nextSkipped = skippedIt.next();
        ...
        return new ArrayList<InetAddressAndPorts>(replicas);
    }

    private static boolean hasSufficientReplicas(String dc, Map<String, Set<InetAddressAndPorts>> dcReplicas, Multimap<String, InetAddressAndPorts> allEndpoints, Map<String, Integer> datacenters)
    {
        return dcReplicas.get(dc).size() >= Math.min(allEndpoints.get(dc).size(), getReplicationFactor(dc, datacenters));
    }

    private static boolean hasSufficientReplicas(Map<String, Set<InetAddressAndPorts>> dcReplicas, Multimap<String, InetAddressAndPorts> allEndpoints, Map<String, Integer> datacenters)
    {
