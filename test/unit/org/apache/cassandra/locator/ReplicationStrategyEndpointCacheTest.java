...
public class ReplicationStrategyEndpointCacheTest
{
    ...
    public void setup(Class stratClass, Map<String, String> strategyOptions) throws Exception
    {
        ...
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(10)), InetAddressAndPorts.getByName("127.0.0.1"));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(20)), InetAddressAndPorts.getByName("127.0.0.2"));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(30)), InetAddressAndPorts.getByName("127.0.0.3"));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(40)), InetAddressAndPorts.getByName("127.0.0.4"));
        //tmd.updateNormalToken(new BigIntegerToken(String.valueOf(50)), InetAddressAndPorts.getByName("127.0.0.5", null, null));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(60)), InetAddressAndPorts.getByName("127.0.0.6"));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(70)), InetAddressAndPorts.getByName("127.0.0.7"));
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(80)), InetAddressAndPorts.getByName("127.0.0.8"));
    }
    ...
    {
        setup(stratClass, configOptions);
        ArrayList<InetAddressAndPorts> initial;
        ArrayList<InetAddressAndPorts> endpoints;
        ...
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(35)), InetAddressAndPorts.getByName("127.0.0.5"));
        endpoints = strategy.getNaturalEndpoints(searchToken);
        assert endpoints.size() == 5 : StringUtils.join(endpoints, ",");
        ...
        initial = strategy.getNaturalEndpoints(searchToken);
        tmd.removeEndpoint(InetAddressAndPorts.getByName("127.0.0.5"));
        endpoints = strategy.getNaturalEndpoints(searchToken);
        assert endpoints.size() == 5 : StringUtils.join(endpoints, ",");
        assert !endpoints.contains(InetAddressAndPorts.getByName("127.0.0.5"));
        assert !endpoints.equals(initial);

        // test token change
        initial = strategy.getNaturalEndpoints(searchToken);
        //move .8 after search token but before other DC3
        tmd.updateNormalToken(new BigIntegerToken(String.valueOf(25)), InetAddressAndPorts.getByName("127.0.0.8"));
        endpoints = strategy.getNaturalEndpoints(searchToken);
        assert endpoints.size() == 5 : StringUtils.join(endpoints, ",");
        assert !endpoints.equals(initial);
    }
    ...
    protected static class FakeSimpleStrategy extends SimpleStrategy
    {
        ...
        public List<InetAddressAndPorts> calculateNaturalEndpoints(Token token, TokenMetadata metadata)
        {
