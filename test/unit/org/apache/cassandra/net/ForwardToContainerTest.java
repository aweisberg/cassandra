...
public class ForwardToContainerTest
{
    ...
    {
        InetAddressAndPorts.initializeDefaultPorts(65532, 65534);
        List<InetAddressAndPorts> addresses = ImmutableList.of(InetAddressAndPorts.getByNameOverrideDefaults("127.0.0.1", 42, 43),
                                                               InetAddressAndPorts.getByName("127.0.0.1"),
                                                               InetAddressAndPorts.getByName("127.0.0.1:7000:7001"),
                                                               InetAddressAndPorts.getByNameOverrideDefaults("2001:0db8:0000:0000:0000:ff00:0042:8329", 42, 43),
                                                               InetAddressAndPorts.getByName("2001:0db8:0000:0000:0000:ff00:0042:8329"),
                                                               InetAddressAndPorts.getByName("[2001:0db8:0000:0000:0000:ff00:0042:8329]:7000:7001"));
        ...
        Iterator<InetAddressAndPorts> iterator = deserialized.targets.iterator();
        if (version >= MessagingService.VERSION_40)
        {
            for (int ii = 0; ii < addresses.size(); ii++)
            {
                InetAddressAndPorts original = addresses.get(ii);
                InetAddressAndPorts roundtripped = iterator.next();
        ...
        else
        {
            for (int ii = 0; ii < addresses.size(); ii++)
            {
                ...
                assertEquals(65534, roundtripped.sslport);
            }
        }
    }
}
