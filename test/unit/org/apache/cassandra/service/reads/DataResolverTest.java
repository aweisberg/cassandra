...
public class DataResolverTest
{
    ...
    private Endpoint peer()
    {
        try
        {
            return Endpoint.getByAddress(new byte[]{ 127, 0, 0, (byte) addressSuffix++ });
        }
    ...
    public MessageIn<ReadResponse> readResponseMessage(Endpoint from, UnfilteredPartitionIterator partitionIterator)
    {
        return readResponseMessage(from, partitionIterator, command);

    }
    public MessageIn<ReadResponse> readResponseMessage(Endpoint from, UnfilteredPartitionIterator partitionIterator, ReadCommand cmd)
    {
