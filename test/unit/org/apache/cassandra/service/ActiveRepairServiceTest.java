...
public class ActiveRepairServiceTest
{
    ...
    public InetAddressAndPorts LOCAL, REMOTE;
    ...
    public void prepare() throws Exception
    {
        if (!initialized)
        {
            ...
            REMOTE = InetAddressAndPorts.getByName("127.0.0.2");
        }
    ...
    public void testGetNeighborsPlusOne() throws Throwable
    {
        ...
        Set<InetAddressAndPorts> expected = addTokens(1 + Keyspace.open(KEYSPACE5).getReplicationStrategy().getReplicationFactor());
        ...
        Set<InetAddressAndPorts> neighbors = new HashSet<>();
    ...
    public void testGetNeighborsTimesTwo() throws Throwable
    {
        ...
        Set<InetAddressAndPorts> expected = new HashSet<>();
    ...
    public void testGetNeighborsPlusOneInLocalDC() throws Throwable
    {
        ...
        HashSet<InetAddressAndPorts> localEndpoints = Sets.newHashSet(topology.getDatacenterEndpoints().get(DatabaseDescriptor.getLocalDataCenter()));
    ...
    public void testGetNeighborsTimesTwoInSpecifiedHosts() throws Throwable
    {
        ...
        addTokens(2 * Keyspace.open(KEYSPACE5).getReplicationStrategy().getReplicationFactor());
        AbstractReplicationStrategy ars = Keyspace.open(KEYSPACE5).getReplicationStrategy();
        List<InetAddressAndPorts> expected = new ArrayList<>();
        for (Range<Token> replicaRange : ars.getAddressRanges().get(FBUtilities.getBroadcastAddressAndPorts()))
        {
            expected.addAll(ars.getRangeAddresses(tmd.cloneOnlyTokenMap()).get(replicaRange));
        }
    ...
    Set<InetAddressAndPorts> addTokens(int max) throws Throwable
    {
        TokenMetadata tmd = StorageService.instance.getTokenMetadata();
        Set<InetAddressAndPorts> endpoints = new HashSet<>();
        for (int i = 1; i <= max; i++)
        {
            InetAddressAndPorts endpoint = InetAddressAndPorts.getByName("127.0.0." + i);
    ...
    public void testGetActiveRepairedSSTableRefs()
    {
        ...
        ActiveRepairService.instance.registerParentRepairSession(prsId, FBUtilities.getBroadcastAddressAndPorts(), Collections.singletonList(store), null, true, 0, false);
    ...
    public void testAddingMoreSSTables()
    {
        ...
        UUID prsId = UUID.randomUUID();
        ActiveRepairService.instance.registerParentRepairSession(prsId, FBUtilities.getBroadcastAddressAndPorts(), Collections.singletonList(store), null, true, System.currentTimeMillis(), true);
        ...
        try
        {
            ...
            ActiveRepairService.instance.registerParentRepairSession(newPrsId, FBUtilities.getBroadcastAddressAndPorts(), Collections.singletonList(store), null, true, System.currentTimeMillis(), true);
    ...
    public void testSnapshotAddSSTables() throws ExecutionException, InterruptedException
    {
        ...
        UUID prsId = UUID.randomUUID();
        Set<SSTableReader> original = Sets.newHashSet(store.select(View.select(SSTableSet.CANONICAL, (s) -> !s.isRepaired())).sstables);
        ActiveRepairService.instance.registerParentRepairSession(prsId, FBUtilities.getBroadcastAddressAndPorts(), Collections.singletonList(store), Collections.singleton(new Range<>(store.getPartitioner().getMinimumToken(), store.getPartitioner().getMinimumToken())), true, System.currentTimeMillis(), true);
        ...
        ActiveRepairService.instance.registerParentRepairSession(prsId2, FBUtilities.getBroadcastAddressAndPorts(), Collections.singletonList(store), Collections.singleton(new Range<>(store.getPartitioner().getMinimumToken(), store.getPartitioner().getMinimumToken())), true, System.currentTimeMillis(), true);
        createSSTables(store, 2);
        ActiveRepairService.instance.getParentRepairSession(prsId).maybeSnapshot(store.metadata.cfId, prsId);
