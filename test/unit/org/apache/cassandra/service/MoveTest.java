...
public class MoveTest
{
    ...
    private static void addNetworkTopologyKeyspace(String keyspaceName, Integer... replicas) throws Exception
    {
        ...
        tmd.clearUnsafe();
        tmd.updateHostId(UUID.randomUUID(), Endpoint.getByName("127.0.0.1"));
        tmd.updateHostId(UUID.randomUUID(), Endpoint.getByName("127.0.0.2"));
    ...
    private void moveHost(Endpoint host, int token, TokenMetadata tmd, VersionedValue.VersionedValueFactory valueFactory )
    {
    ...
    private void finishMove(Endpoint host, int token, TokenMetadata tmd)
    {
    ...
    private Map.Entry<Range<Token>, Collection<Endpoint>> generatePendingMapEntry(int start, int end, String... endpoints) throws UnknownHostException
    {
        Map<Range<Token>, Collection<Endpoint>> pendingRanges = new HashMap<>();
        pendingRanges.put(generateRange(start, end), makeAddrs(endpoints));
        return pendingRanges.entrySet().iterator().next();
    }

    private Map<Range<Token>, Collection<Endpoint>> generatePendingRanges(Map.Entry<Range<Token>, Collection<Endpoint>>... entries)
    {
        Map<Range<Token>, Collection<Endpoint>> pendingRanges = new HashMap<>();
        for(Map.Entry<Range<Token>, Collection<Endpoint>> entry : entries)
        {
    ...
    private void assertPendingRanges(TokenMetadata tmd, Map<Range<Token>,  Collection<Endpoint>> pendingRanges, String keyspaceName) throws ConfigurationException
    {
    ...
    private void assertMaps(Map<Range<Token>, Collection<Endpoint>> expected, PendingRangeMaps actual)
    {
        int sizeOfActual = 0;
        Iterator<Map.Entry<Range<Token>, List<Endpoint>>> iterator = actual.iterator();
        while(iterator.hasNext())
        {
            Map.Entry<Range<Token>, List<Endpoint>> actualEntry = iterator.next();
    ...
    public void newTestWriteEndpointsDuringMove() throws Exception
    {
        ...
        Map<Token, List<Endpoint>> expectedEndpoints = new HashMap<>();
        ...
        for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces())
        {
            ...
            for (Token token : keyTokens)
            {
                ...
                HashSet<Endpoint> actual = new HashSet<>(tmd.getWriteEndpoints(token, keyspaceName, strategy.calculateNaturalEndpoints(token, tmd.cloneOnlyTokenMap())));
                HashSet<Endpoint> expected = new HashSet<>();
    ...
    public void testSimultaneousMove() throws UnknownHostException
    {
        ...
        Endpoint boot1 = Endpoint.getByAddressOverrideDefaults(InetAddress.getByName("127.0.1.1"), null, UUID.randomUUID());
        ...
        Endpoint boot2 = Endpoint.getByAddressOverrideDefaults(InetAddress.getByName("127.0.1.2"), null, UUID.randomUUID());
        ...
        */
        ...
        Multimap<Endpoint, Range<Token>> keyspace1ranges = keyspaceStrategyMap.get(Simple_RF1_KeyspaceName).getAddressRanges();
        Collection<Range<Token>> ranges1 = keyspace1ranges.get(Endpoint.getByName("127.0.0.1"));
        assertEquals(1, collectionSize(ranges1));
        assertEquals(generateRange(97, 0), ranges1.iterator().next());
        Collection<Range<Token>> ranges2 = keyspace1ranges.get(Endpoint.getByName("127.0.0.2"));
        assertEquals(1, collectionSize(ranges2));
        assertEquals(generateRange(0, 10), ranges2.iterator().next());
        Collection<Range<Token>> ranges3 = keyspace1ranges.get(Endpoint.getByName("127.0.0.3"));
        assertEquals(1, collectionSize(ranges3));
        assertEquals(generateRange(10, 20), ranges3.iterator().next());
        Collection<Range<Token>> ranges4 = keyspace1ranges.get(Endpoint.getByName("127.0.0.4"));
        assertEquals(1, collectionSize(ranges4));
        assertEquals(generateRange(20, 30), ranges4.iterator().next());
        Collection<Range<Token>> ranges5 = keyspace1ranges.get(Endpoint.getByName("127.0.0.5"));
        assertEquals(1, collectionSize(ranges5));
        assertEquals(generateRange(30, 40), ranges5.iterator().next());
        Collection<Range<Token>> ranges6 = keyspace1ranges.get(Endpoint.getByName("127.0.0.6"));
        assertEquals(1, collectionSize(ranges6));
        assertEquals(generateRange(40, 50), ranges6.iterator().next());
        Collection<Range<Token>> ranges7 = keyspace1ranges.get(Endpoint.getByName("127.0.0.7"));
        assertEquals(1, collectionSize(ranges7));
        assertEquals(generateRange(50, 67), ranges7.iterator().next());
        Collection<Range<Token>> ranges8 = keyspace1ranges.get(Endpoint.getByName("127.0.0.8"));
        assertEquals(1, collectionSize(ranges8));
        assertEquals(generateRange(67, 70), ranges8.iterator().next());
        Collection<Range<Token>> ranges9 = keyspace1ranges.get(Endpoint.getByName("127.0.0.9"));
        assertEquals(1, collectionSize(ranges9));
        assertEquals(generateRange(70, 87), ranges9.iterator().next());
        Collection<Range<Token>> ranges10 = keyspace1ranges.get(Endpoint.getByName("127.0.0.10"));
        ...
        */
        ...
        expectedEndpoints.put(KEYSPACE2, HashMultimap.<Token, Endpoint>create());
        ...
        expectedEndpoints.put(KEYSPACE3, HashMultimap.<Token, Endpoint>create());
        ...
        expectedEndpoints.put(Simple_RF3_KeyspaceName, HashMultimap.<Token, Endpoint>create());
    ...
    private static Collection<Endpoint> makeAddrs(String... hosts) throws UnknownHostException
    {
