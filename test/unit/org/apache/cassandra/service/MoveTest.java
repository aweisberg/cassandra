...
public class MoveTest
{
    ...
    private static void addNetworkTopologyKeyspace(String keyspaceName, Integer... replicas) throws Exception
    {
        ...
        DatabaseDescriptor.setEndpointSnitch(new AbstractNetworkTopologySnitch()
        {
            ...
            private int getIPLastPart(InetAddressAndPorts endpoint)
            {
        ...
        final TokenMetadata tmd = StorageService.instance.getTokenMetadata();
                tmd.clearUnsafe();
                tmd.updateHostId(UUID.randomUUID(), InetAddressAndPorts.getByName("127.0.0.1"));
                tmd.updateHostId(UUID.randomUUID(), InetAddressAndPorts.getByName("127.0.0.2"));
    ...
    public void testMoveWithPendingRangesNetworkStrategyRackAwareThirtyNodes() throws Exception
    {
        ...
        List<InetAddressAndPorts> hosts = new ArrayList<>();
    ...
    private void moveHost(InetAddressAndPorts host, int token, TokenMetadata tmd, VersionedValue.VersionedValueFactory valueFactory )
    {
    ...
    private void finishMove(InetAddressAndPorts host, int token, TokenMetadata tmd)
    {
    ...
    private Map.Entry<Range<Token>, Collection<InetAddressAndPorts>> generatePendingMapEntry(int start, int end, String... endpoints) throws UnknownHostException
    {
        Map<Range<Token>, Collection<InetAddressAndPorts>> pendingRanges = new HashMap<>();
        pendingRanges.put(generateRange(start, end), makeAddrs(endpoints));
        return pendingRanges.entrySet().iterator().next();
    }

    private Map<Range<Token>, Collection<InetAddressAndPorts>> generatePendingRanges(Map.Entry<Range<Token>, Collection<InetAddressAndPorts>>... entries)
    {
        Map<Range<Token>, Collection<InetAddressAndPorts>> pendingRanges = new HashMap<>();
        for(Map.Entry<Range<Token>, Collection<InetAddressAndPorts>> entry : entries)
        {
    ...
    private void assertPendingRanges(TokenMetadata tmd, Map<Range<Token>,  Collection<InetAddressAndPorts>> pendingRanges, String keyspaceName) throws ConfigurationException
    {
    ...
    private void assertMaps(Map<Range<Token>, Collection<InetAddressAndPorts>> expected, PendingRangeMaps actual)
    {
        int sizeOfActual = 0;
        Iterator<Map.Entry<Range<Token>, List<InetAddressAndPorts>>> iterator = actual.iterator();
        while(iterator.hasNext())
        {
            Map.Entry<Range<Token>, List<InetAddressAndPorts>> actualEntry = iterator.next();
    ...
    public void newTestWriteEndpointsDuringMove() throws Exception
    {
        ...
        Map<Token, List<InetAddressAndPorts>> expectedEndpoints = new HashMap<>();
        for (Token token : keyTokens)
        {
            List<InetAddressAndPorts> endpoints = new ArrayList<>();
        ...
        for (String keyspaceName : Schema.instance.getNonLocalStrategyKeyspaces())
        {
            ...
            for (Token token : keyTokens)
            {
                ...
                HashSet<InetAddressAndPorts> actual = new HashSet<>(tmd.getWriteEndpoints(token, keyspaceName, strategy.calculateNaturalEndpoints(token, tmd.cloneOnlyTokenMap())));
                HashSet<InetAddressAndPorts> expected = new HashSet<>();
    ...
    public void testSimultaneousMove() throws UnknownHostException
    {
        ...
        Collection<InetAddressAndPorts> endpoints;

        tmd = tmd.cloneAfterAllSettled();
        ss.setTokenMetadataUnsafe(tmd);

        // boot two new nodes with keyTokens.get(5) and keyTokens.get(7)
        InetAddressAndPorts boot1 = InetAddressAndPorts.getByName("127.0.1.1");
        ...
        InetAddressAndPorts boot2 = InetAddressAndPorts.getByName("127.0.1.2");
        ...
        */
        ...
        Multimap<InetAddressAndPorts, Range<Token>> keyspace1ranges = keyspaceStrategyMap.get(Simple_RF1_KeyspaceName).getAddressRanges();
        Collection<Range<Token>> ranges1 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.1"));
        assertEquals(1, collectionSize(ranges1));
        assertEquals(generateRange(97, 0), ranges1.iterator().next());
        Collection<Range<Token>> ranges2 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.2"));
        assertEquals(1, collectionSize(ranges2));
        assertEquals(generateRange(0, 10), ranges2.iterator().next());
        Collection<Range<Token>> ranges3 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.3"));
        assertEquals(1, collectionSize(ranges3));
        assertEquals(generateRange(10, 20), ranges3.iterator().next());
        Collection<Range<Token>> ranges4 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.4"));
        assertEquals(1, collectionSize(ranges4));
        assertEquals(generateRange(20, 30), ranges4.iterator().next());
        Collection<Range<Token>> ranges5 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.5"));
        assertEquals(1, collectionSize(ranges5));
        assertEquals(generateRange(30, 40), ranges5.iterator().next());
        Collection<Range<Token>> ranges6 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.6"));
        assertEquals(1, collectionSize(ranges6));
        assertEquals(generateRange(40, 50), ranges6.iterator().next());
        Collection<Range<Token>> ranges7 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.7"));
        assertEquals(1, collectionSize(ranges7));
        assertEquals(generateRange(50, 67), ranges7.iterator().next());
        Collection<Range<Token>> ranges8 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.8"));
        assertEquals(1, collectionSize(ranges8));
        assertEquals(generateRange(67, 70), ranges8.iterator().next());
        Collection<Range<Token>> ranges9 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.9"));
        assertEquals(1, collectionSize(ranges9));
        assertEquals(generateRange(70, 87), ranges9.iterator().next());
        Collection<Range<Token>> ranges10 = keyspace1ranges.get(InetAddressAndPorts.getByName("127.0.0.10"));
        ...
        */
         ...
         *  }
         */
        Multimap<InetAddressAndPorts, Range<Token>> keyspace4ranges = keyspaceStrategyMap.get(Simple_RF3_KeyspaceName).getAddressRanges();
        ranges1 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.1"));
        assertEquals(collectionSize(ranges1), 3);
        assertTrue(ranges1.equals(generateRanges(97, 0, 70, 87, 87, 97)));
        ranges2 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.2"));
        assertEquals(collectionSize(ranges2), 3);
        assertTrue(ranges2.equals(generateRanges(97, 0, 87, 97, 0, 10)));
        ranges3 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.3"));
        assertEquals(collectionSize(ranges3), 3);
        assertTrue(ranges3.equals(generateRanges(97, 0, 0, 10, 10, 20)));
        ranges4 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.4"));
        assertEquals(collectionSize(ranges4), 3);
        assertTrue(ranges4.equals(generateRanges(20, 30, 0, 10, 10, 20)));
        ranges5 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.5"));
        assertEquals(collectionSize(ranges5), 3);
        assertTrue(ranges5.equals(generateRanges(30, 40, 20, 30, 10, 20)));
        ranges6 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.6"));
        assertEquals(collectionSize(ranges6), 3);
        assertTrue(ranges6.equals(generateRanges(40, 50, 30, 40, 20, 30)));
        ranges7 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.7"));
        assertEquals(collectionSize(ranges7), 3);
        assertTrue(ranges7.equals(generateRanges(40, 50, 30, 40, 50, 67)));
        ranges8 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.8"));
        assertEquals(collectionSize(ranges8), 3);
        assertTrue(ranges8.equals(generateRanges(40, 50, 50, 67, 67, 70)));
        ranges9 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.9"));
        assertEquals(collectionSize(ranges9), 3);
        assertTrue(ranges9.equals(generateRanges(70, 87, 50, 67, 67, 70)));
        ranges10 = keyspace4ranges.get(InetAddressAndPorts.getByName("127.0.0.10"));
        assertEquals(collectionSize(ranges10), 3);
        assertTrue(ranges10.equals(generateRanges(70, 87, 87, 97, 67, 70)));

        // pre-calculate the results.
        Map<String, Multimap<Token, InetAddressAndPorts>> expectedEndpoints = new HashMap<>();
        expectedEndpoints.put(Simple_RF1_KeyspaceName, HashMultimap.<Token, InetAddressAndPorts>create());
        ...
        expectedEndpoints.put(KEYSPACE2, HashMultimap.<Token, InetAddressAndPorts>create());
        ...
        expectedEndpoints.put(KEYSPACE3, HashMultimap.<Token, InetAddressAndPorts>create());
        ...
        expectedEndpoints.put(Simple_RF3_KeyspaceName, HashMultimap.<Token, InetAddressAndPorts>create());
    ...
    private static Collection<InetAddressAndPorts> makeAddrs(String... hosts) throws UnknownHostException
    {
        ArrayList<InetAddressAndPorts> addrs = new ArrayList<>(hosts.length);
        for (String host : hosts)
            addrs.add(InetAddressAndPorts.getByName(host));
        return addrs;
    }
