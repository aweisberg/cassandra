...
public class CassandraIndexTest extends CQLTester
{
    ...
    public void indexOnStaticColumnWithRestrictedClustering() throws Throwable
    {
        ...
        // restriction on the base clustering columns can not be pushed down into the index read. When it was,
        // every single partition read that also restricted the clustering silently returned no rows at all.
        for (String order : new String[]{ "ASC", "DESC" })
        {
            ...
            for (boolean flushed : new boolean[]{ false, true })
            {
                ...
                assertRowsIgnoringOrder(execute("SELECT k, c, v FROM %s WHERE k = 0 AND c > 1 AND s = 9"),
                                        row(0, 2, 2), row(0, 3, 3));
                // no clustering restriction at all, which always worked
    ...
    public void indexOnPartitionKeyColumnWithUpdatedStaticRow() throws Throwable
    {
        ...
        // why insertRow lets the static row through. updateRow did not, so the entry for the static row was only
        // ever written when the static row was first inserted: a partition whose static row was created by
        // something that left no live cell behind, and made live by a later update merging into it, never got an
        // entry at all and the query skipped the partition entirely.
