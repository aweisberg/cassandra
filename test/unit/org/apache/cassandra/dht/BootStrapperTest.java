...
public class BootStrapperTest
{
    ...
    private RangeStreamer testSourceTargetComputation(String keyspaceName, int numOldNodes, int replicationFactor) throws UnknownHostException
    {
        ...
        Endpoint myEndpoint = Endpoint.getByName("127.0.0.1");
        ...
        IFailureDetector mockFailureDetector = new IFailureDetector()
        {
            ...
            public void interpret(Endpoint ep) { throw new UnsupportedOperationException(); }
            public void report(Endpoint ep) { throw new UnsupportedOperationException(); }
            public void registerFailureDetectionEventListener(IFailureDetectionEventListener listener) { throw new UnsupportedOperationException(); }
            public void unregisterFailureDetectionEventListener(IFailureDetectionEventListener listener) { throw new UnsupportedOperationException(); }
            public void remove(Endpoint ep) { throw new UnsupportedOperationException(); }
            public void forceConviction(Endpoint ep) { throw new UnsupportedOperationException(); }
        };
        s.addSourceFilter(new RangeStreamer.FailureDetectorSourceFilter(mockFailureDetector));
        s.addRanges(keyspaceName, Keyspace.open(keyspaceName).getReplicationStrategy().getPendingAddressRanges(tmd, myToken, myEndpoint));

        Collection<Map.Entry<Endpoint, Collection<Range<Token>>>> toFetch = s.toFetch().get(keyspaceName);

        // Check we get get RF new ranges in total
        Set<Range<Token>> ranges = new HashSet<>();
        for (Map.Entry<Endpoint, Collection<Range<Token>>> e : toFetch)
    ...
    private void generateFakeEndpoints(TokenMetadata tmd, int numOldNodes, int numVNodes, String dc, String rack) throws UnknownHostException
    {
        ...
        for (int i = 1; i <= numOldNodes; i++)
        {
            ...
            Endpoint addr = Endpoint.getByName("127." + dc + "." + rack + "." + (i + 1));
    ...
    public void testAllocateTokens() throws UnknownHostException
    {
        ...
        Endpoint addr = FBUtilities.getBroadcastAddressAndPort();
    ...
    public void testAllocateTokensNetworkStrategy(int rackCount, int replicas) throws UnknownHostException
    {
        ...
        try
        {
            ...
            metadata.updateHostId(UUID.randomUUID(), Endpoint.getByName("127.1.0.99"));
            metadata.updateHostId(UUID.randomUUID(), Endpoint.getByName("127.15.0.99"));
            ...
            Endpoint addr = Endpoint.getByName("127." + dc + ".0.99");
            allocateTokensForNode(vn, ks, tm, addr);
    ...
    private void allocateTokensForNode(int vn, String ks, TokenMetadata tm, Endpoint addr)
    {
    ...
    public void testAllocateTokensMultipleKeyspaces() throws UnknownHostException
    {
        ...
        Endpoint dcaddr = FBUtilities.getBroadcastAddressAndPort();
        ...
        for (int i=11; i<=20; ++i)
        {
            allocateTokensForNode(vn, cks, tm, Endpoint.getByName("127.0.0." + (i + 1)));
