...
public class StreamStateStoreTest
{
    ...
    public void testUpdateAndQueryAvailableRanges()
    {
        ...
        session.addStreamRequest("keyspace1", ReplicaList.toDummyList(Collections.singleton(range)), Collections.singleton("cf"));
        ...
        session.addStreamRequest("keyspace1", ReplicaList.toDummyList(Collections.singleton(range2)), Collections.singleton("cf"));
        session.state(StreamSession.State.COMPLETE);
        store.handleStreamEvent(new StreamEvent.SessionCompleteEvent(session));
