...
public class TrackedRangeReadTest extends TrackedRangeReadTestBase
{
    ...
    /**
     * An indexed range read whose predicate is a partition key column and a static column, both of which SAI answers
     * from an index. (1,'b') satisfies the expression on the partition key column and not the one on the static
    ...
    {
        staticOnlyPartitionDoesNotMatch("g_indexed_static_only", TABLE_WITH_INDEXED_STATIC,
                                        "SELECT pk0, pk1, ck, s, v FROM %s.tbl WHERE pk0 = 1 AND s = 7");
    }
    ...
    {
        staticOnlyPartitionsAreDropped("g_indexed_static_only_paged", TABLE_WITH_INDEXED_STATIC,
                                       "SELECT pk0, pk1, ck, s, v FROM %s.tbl WHERE pk0 = 1 AND s = 7", 1);
    }
    ...
    {
        staticOnlyPartitionsAreDropped("g_indexed_static_only_limited", TABLE_WITH_INDEXED_STATIC,
                                       "SELECT pk0, pk1, ck, s, v FROM %s.tbl WHERE pk0 = 1 AND s = 7 LIMIT 1", UNPAGED);
    }
    ...
    /** The index scan is stopped by a page limit and reconciliation then hands it a key past everything it scanned. */
    @Test
