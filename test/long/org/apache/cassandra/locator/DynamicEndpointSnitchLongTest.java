...
public class DynamicEndpointSnitchLongTest
{
    ...
    public void testConcurrency() throws InterruptedException, IOException, ConfigurationException
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 100; i++)
                for (int j = 0; j < 256; j++)
                    hosts.add(InetAddressAndPorts.getByAddress(new byte[]{127, 0, (byte)i, (byte)j}));

            ScoreUpdater updater = new ScoreUpdater(dsnitch, hosts);
            updater.start();

            List<InetAddressAndPorts> result = null;
    ...
    public static class ScoreUpdater extends Thread
    {
        ...
        private final List<InetAddressAndPorts> hosts;
        private final Random random = new Random();

        public ScoreUpdater(DynamicEndpointSnitch dsnitch, List<InetAddressAndPorts> hosts)
        {
        ...
        public void run()
        {
            while (!stopped)
            {
                InetAddressAndPorts host = hosts.get(random.nextInt(hosts.size()));
