...
public class DynamicEndpointSnitchLongTest
{
    ...
    public void testConcurrency() throws InterruptedException, IOException, ConfigurationException
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 100; i++)
                for (int j = 0; j < 256; j++)
                    hosts.add(Endpoint.getByAddress(new byte[]{ 127, 0, (byte)i, (byte)j}));
    ...
    public static class ScoreUpdater extends Thread
    {
        ...
        public ScoreUpdater(DynamicEndpointSnitch dsnitch, List<Endpoint> hosts)
        {
        ...
        public void run()
        {
            while (!stopped)
            {
                Endpoint host = hosts.get(random.nextInt(hosts.size()));
