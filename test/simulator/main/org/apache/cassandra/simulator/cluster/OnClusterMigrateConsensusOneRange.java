...
class OnClusterMigrateConsensusOneRange extends Action
{
    ...
    {
        System.out.println("Running OnClusterMigrateConsensusOneRange range " + startMigrationRange);
        ...
        System.out.println("Migrating via repair " + migrateViaRepair);
        ...
        {
            if (migrateViaRepair)
                result.add(new OnInstanceRepair(actions, repairOn, RepairType.ACCORD, startMigrationRange, false));
            else
        ...
        else
        {
            if (migrateViaRepair)
            {
                result.add(new OnInstanceRepair(actions, repairOn, RepairType.DATA, startMigrationRange, false));
                result.add(new OnInstanceRepair(actions, repairOn, RepairType.DATA_AND_PAXOS, startMigrationRange, false));
            }
            else
            {
                result.add(new OnInstanceFinishConsensusMigration(actions, repairOn, rangeString, targetString));
