...
public class OnInstanceRepair extends ClusterAction
{
    public enum RepairType
    {
        ...
        PAXOS(false, true, false),
        DATA_AND_PAXOS(true, true, false),
        ACCORD(false, false, true);

        public final boolean repairData;
        public final boolean repairPaxos;
        public final boolean repairAccord;

        RepairType(boolean repairData, boolean repairPaxos, boolean repairAccord)
        {
    ...
    {
        return () -> {
            System.out.printf("Invoking blocking repair of keyspace %s repairType %s, repairRange %s, force %b%n", keyspaceName, repairType, repairRange, force);
            Condition done = newOneTimeCondition();
            invokeRepair(keyspaceName, repairType, () -> parseTokenRanges(singletonList(repairRange)), false, force, done::signal);
            done.awaitThrowUncheckedOnInterrupt();
            System.out.println("Completed blocking repair");
        };
    }
    ...
    private static void invokeRepair(String keyspaceName, RepairType repairType, IIsolatedExecutor.SerializableCallable<Collection<Range<Token>>> rangesSupplier, boolean isPrimaryRangeOnly, boolean force, Runnable listener)
    {
        ...
        StorageService.instance.repair(keyspaceName, new RepairOption(RepairParallelism.SEQUENTIAL, isPrimaryRangeOnly, false, false, 1, ranges, false, force, PreviewKind.NONE, false, true, repairType.repairData, repairType.repairPaxos, repairType.repairAccord), singletonList((tag, event) -> {
